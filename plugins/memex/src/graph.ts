...
import { openGraphCypher, type GraphIndexPort, type GraphCypherTier, type CypherParam } from "./graph-index.js";
...
export interface GraphOperationBase { root: string; homeDir?: string; limit?: number; responseByteLimit?: number; }
export interface BuildGraphOptions extends GraphOperationBase { now?: string; force?: boolean; limits?: { maxFiles?: number; maxFileBytes?: number; maxRepositoryBytes?: number }; }
export interface TargetGraphOptions extends GraphOperationBase { target: string; }
export interface ImpactGraphOptions extends TargetGraphOptions { direction?: "inbound" | "outbound" | "both"; depth?: number; }
export interface QueryGraphOptions extends GraphOperationBase { query: string; }
export interface GraphResultEnvelope { schemaVersion: 1; action: "query" | "context"; root: string; nodes: GraphNodeV1[]; edges: GraphEdgeV1[]; truncated: boolean; diagnostics: GraphDiagnosticV1[]; }
export interface GraphImpactEnvelope extends Omit<GraphResultEnvelope, "action"> { action: "impact"; direction: "inbound" | "outbound" | "both"; depth: number; paths: ImpactResult["paths"]; }
export interface GraphChangesEnvelope extends Omit<GraphResultEnvelope, "action"> { action: "changes"; changedPaths: string[]; changeState: "working-tree" | "committed" | "clean"; head?: string; base?: string; }
export interface BuildGraphResult { schemaVersion: 1; action: "build"; root: string; fresh: true; buildMode: "full" | "incremental"; fullRebuild: boolean; head?: string; previousHead?: string; dirtyFingerprint: string; changedPaths: string[]; truncated: boolean; scannedFileCount: number; removedFileCount: number; fileCount: number; nodeCount: number; edgeCount: number; diagnosticCount: number; diagnostics: GraphDiagnosticV1[]; generatedAt: string; }
export interface GraphStatusResult { schemaVersion: 1; action: "status"; root: string; available: boolean; fresh: boolean; reason?: string; indexedHead?: string; currentHead?: string; counts?: { files: number; nodes: number; edges: number; diagnostics: number }; diagnostics?: GraphDiagnosticV1[]; generatedAt?: string; }
...
export async function buildGraph(options: BuildGraphOptions): Promise<BuildGraphResult> {
  ...
  for (const file of metadata) {
    ...
    catch (error) { if (error instanceof MemexError && error.code === "UNSUPPORTED_SOURCE") continue; throw error; }
...
export async function getGraphStatus(options: GraphOperationBase): Promise<GraphStatusResult> {
  ...
  if (!probed.initialized) return { schemaVersion: 1, action: "status", root: probed.repositoryRoot, available: false, fresh: false, reason: "No recoverable Memex graph manifest exists." };
  let manifest;
  try { manifest = await readManifest(probed.storage); }
  catch { return { schemaVersion: 1, action: "status", root: probed.repositoryRoot, available: false, fresh: false, reason: "No recoverable Memex graph manifest exists." }; }
...
export async function queryGraphOperation(options: QueryGraphOptions): Promise<GraphResultEnvelope & { query: string }> { const loaded = await loadIndex(options); const result = await lazyCandidates(loaded.index, options.query, options.limit, options.responseByteLimit, loaded.diagnostics); return boundedEnvelope(envelope("query", loaded.root, loaded.diagnostics, result, { query: options.query }), options.responseByteLimit); }
export { queryGraphOperation as queryGraph };
export async function getGraphContext(options: TargetGraphOptions): Promise<GraphResultEnvelope & { target: string }> { const loaded = await loadIndex(options); const roots = await lazyTargetNodes(loaded.index, options.target, options.limit); const ids = new Set(roots.map((node) => node.id)); const adjacency = await adjacentEdges(loaded.index, ids, entityLimit(options.limit)); for (const edge of adjacency.edges) { ids.add(edge.from); ids.add(edge.to); } const result = await lazyResult(loaded.index, ids, adjacency.edges, options.limit, options.responseByteLimit, loaded.diagnostics); return boundedEnvelope(envelope("context", loaded.root, loaded.diagnostics, { ...result, truncated: result.truncated || adjacency.truncated }, { target: options.target }), options.responseByteLimit); }
export async function analyzeGraphImpact(options: ImpactGraphOptions): Promise<GraphImpactEnvelope & { target: string }> { const loaded = await loadIndex(options); const direction = options.direction ?? "both"; const depth = options.depth ?? GRAPH_DEFAULTS.maxTraversalDepth; const roots = await lazyTargetNodes(loaded.index, options.target, options.limit); const impact = await lazyImpact(loaded.index, roots.map((node) => node.id), direction, depth, options.limit, options.responseByteLimit, loaded.diagnostics); const value: GraphImpactEnvelope & { target: string } = { schemaVersion: 1, action: "impact", root: loaded.root, nodes: impact.nodes, edges: impact.edges, truncated: impact.truncated, diagnostics: loaded.diagnostics, target: options.target, direction, depth, paths: impact.paths }; return boundedEnvelope(value, options.responseByteLimit); }
export async function analyzeGraphChanges(options: GraphOperationBase & { base?: string }): Promise<GraphChangesEnvelope> { const loaded = await loadIndex(options); const evidence = await changedRepositoryEvidence(loaded.root, options.base); const changedIds = await loaded.index.changedPathSeeds(evidence.paths, entityLimit(options.limit)); const result = await lazyImpact(loaded.index, changedIds, "inbound", GRAPH_DEFAULTS.maxTraversalDepth, options.limit, options.responseByteLimit, loaded.diagnostics); const boundedChangedPaths = boundedPaths(evidence.paths, options.limit); const value: GraphChangesEnvelope = { schemaVersion: 1, action: "changes", root: loaded.root, nodes: result.nodes, edges: result.edges, truncated: result.truncated || boundedChangedPaths.truncated, diagnostics: loaded.diagnostics, changedPaths: boundedChangedPaths.values, changeState: evidence.changeState, ...(evidence.head === undefined ? {} : { head: evidence.head }), ...(options.base === undefined ? {} : { base: options.base }) }; return boundedEnvelope(value, options.responseByteLimit); }
export async function getArchitectureMap(options: GraphOperationBase): Promise<{ schemaVersion: 1; action: "map"; root: string; modules: GraphNodeV1[]; entrypoints: GraphNodeV1[]; hubs: GraphNodeV1[]; cycles: string[][]; flows: Array<{ from: string; to: string; weight: number }>; truncated: boolean; truncatedCollections: { modules: boolean; entrypoints: boolean; hubs: boolean; cycles: boolean; flows: boolean; diagnostics: boolean }; diagnostics: GraphDiagnosticV1[] }> { const loaded = await loadIndex(options); const summary = await loaded.index.architectureSummary(); const max = entityLimit(options.limit); const entrypoints = await nodesByIds(loaded.index, summary.entrypoints); const hubs = await nodesByIds(loaded.index, summary.hubs.map((hub) => hub.id)); const flows = await lazyFlows(loaded.index, summary.flows); const cycles = await lazyCycles(loaded.index, summary.cycles); const value = { schemaVersion: 1 as const, action: "map" as const, root: loaded.root, modules: summary.modules.slice(0, max), entrypoints: entrypoints.slice(0, max), hubs: hubs.slice(0, max), cycles: cycles.slice(0, max), flows: flows.slice(0, max), truncated: summary.modules.length > max || entrypoints.length > max || hubs.length > max || cycles.length > max || flows.length > max, truncatedCollections: { modules: summary.modules.length > max, entrypoints: entrypoints.length > max, hubs: hubs.length > max, cycles: cycles.length > max, flows: flows.length > max, diagnostics: false }, diagnostics: loaded.diagnostics }; return boundedEnvelope(value, options.responseByteLimit); }

async function loadIndex(options: GraphOperationBase): Promise<{ root: string; index: GraphIndexPort; diagnostics: GraphDiagnosticV1[] }> { const resolved = await resolveGraphStorage(options.root, options.homeDir); const index = await openGraphIndex(resolved.storage); return { root: resolved.repositoryRoot, index, diagnostics: (await index.architectureSummary()).diagnostics }; }
function envelope<Extra extends object>(action: GraphResultEnvelope["action"], root: string, diagnostics: GraphDiagnosticV1[], result: GraphResult, extra: Extra): GraphResultEnvelope & Extra { return { schemaVersion: 1, action, root, nodes: result.nodes, edges: result.edges, truncated: result.truncated, diagnostics, ...extra }; }

async function lazyCandidates(index: GraphIndexPort, query: string, limit: number | undefined, byteLimit: number | undefined, diagnostics: GraphDiagnosticV1[]): Promise<GraphResult> { if (query.trim().length === 0) throw new MemexError("INVALID_ARGUMENT", "Graph query must not be empty."); const ids = await index.rankedCandidates(query, entityLimit(limit)); const adjacency = await adjacentEdges(index, new Set(ids), entityLimit(limit)); const result = await lazyResult(index, new Set(ids), adjacency.edges, limit, byteLimit, diagnostics); return { ...result, truncated: result.truncated || adjacency.truncated }; }
async function lazyTargetNodes(index: GraphIndexPort, target: string, limit: number | undefined): Promise<GraphNodeV1[]> { const candidates = await index.rankedCandidates(target, entityLimit(limit)); const nodes = await nodesByIds(index, candidates); const needle = target.toLowerCase(); const exact = nodes.filter((node) => node.id === target || node.path === target || node.name.toLowerCase() === needle || `${node.scope ?? ""}.${node.name}`.replace(/^\./u, "").toLowerCase() === needle); if (exact.length > 0) return exact; if (nodes.length === 0) throw new MemexError("NOT_FOUND", "Graph target was not found."); return nodes.slice(0, 1); }
async function nodesByIds(index: GraphIndexPort, ids: readonly string[]): Promise<GraphNodeV1[]> { const nodes = await Promise.all([...new Set(ids)].map((id) => index.node(id))); return nodes.filter((node): node is GraphNodeV1 => node !== undefined).sort((left, right) => left.id.localeCompare(right.id)); }
async function adjacentEdges(index: GraphIndexPort, ids: ReadonlySet<string>, limit: number): Promise<{ edges: GraphEdgeV1[]; truncated: boolean }> { const edges = new Map<string, GraphEdgeV1>(); let truncated = false; for (const id of [...ids].sort((left, right) => left.localeCompare(right))) { const [inbound, outbound] = await Promise.all([index.inbound(id, limit), index.outbound(id, limit)]); truncated ||= inbound.truncated || outbound.truncated; for (const edge of [...inbound.edges, ...outbound.edges]) edges.set(edge.id, edge); } return { edges: [...edges.values()].sort((left, right) => left.id.localeCompare(right.id)), truncated }; }
async function lazyResult(index: GraphIndexPort, ids: ReadonlySet<string>, edges: readonly GraphEdgeV1[], limit: number | undefined, byteLimit: number | undefined, diagnostics: GraphDiagnosticV1[]): Promise<GraphResult> { const max = entityLimit(limit); let nodes = (await nodesByIds(index, [...ids])).slice(0, max); let resultEdges = containedEdges(edges, nodes); let truncated = ids.size > max; while (nodes.length > 0 && Buffer.byteLength(JSON.stringify({ nodes, edges: resultEdges }), "utf8") > responseLimit(byteLimit)) { nodes = nodes.slice(0, -1); resultEdges = containedEdges(edges, nodes); truncated = true; } return { nodes, edges: resultEdges, truncated, unresolvedEdgeCount: diagnostics.filter((diagnostic) => diagnostic.code.startsWith("UNRESOLVED") || diagnostic.code.startsWith("AMBIGUOUS")).length }; }
async function lazyImpact(index: GraphIndexPort, rootIds: readonly string[], direction: "inbound" | "outbound" | "both", depth: number, limit: number | undefined, byteLimit: number | undefined, diagnostics: GraphDiagnosticV1[]): Promise<ImpactResult> { if (!Number.isSafeInteger(depth) || depth < 1 || depth > GRAPH_DEFAULTS.maxTraversalDepth) throw new MemexError("INVALID_ARGUMENT", "Graph traversal depth must be between 1 and 5."); const visited = new Set(rootIds); const paths: ImpactResult["paths"] = rootIds.map((nodeId) => ({ nodeId, depth: 0 })); const edges = new Map<string, GraphEdgeV1>(); let frontier = [...visited].sort((left, right) => left.localeCompare(right)); let traversalTruncated = false; const deadline = Date.now() + LAZY_TRAVERSAL_DEADLINE_MS; for (let currentDepth = 1; currentDepth <= depth && frontier.length > 0 && !traversalTruncated; currentDepth += 1) { const next = new Set<string>(); for (const id of frontier) { if (Date.now() >= deadline || visited.size >= LAZY_TRAVERSAL_MAX_VISITED) { traversalTruncated = true; break; } const sets = direction === "inbound" ? [await index.inbound(id, 100)] : direction === "outbound" ? [await index.outbound(id, 100)] : await Promise.all([index.inbound(id, 100), index.outbound(id, 100)]); for (const set of sets) { traversalTruncated ||= set.truncated; for (const edge of set.edges) { edges.set(edge.id, edge); const nodeId = direction === "inbound" ? edge.from : direction === "outbound" ? edge.to : edge.from === id ? edge.to : edge.from; if (!visited.has(nodeId)) { visited.add(nodeId); next.add(nodeId); paths.push({ nodeId, depth: currentDepth, via: edge.id }); } } } } frontier = [...next].sort((left, right) => left.localeCompare(right)); if (currentDepth === depth && frontier.length > 0) traversalTruncated = true; } const result = await lazyResult(index, visited, [...edges.values()], limit, byteLimit, diagnostics); const included = new Set(result.nodes.map((node) => node.id)); return { ...result, truncated: result.truncated || traversalTruncated, paths: paths.filter((entry) => included.has(entry.nodeId)).sort((left, right) => left.depth - right.depth || left.nodeId.localeCompare(right.nodeId)) }; }
function containedEdges(edges: readonly GraphEdgeV1[], nodes: readonly GraphNodeV1[]): GraphEdgeV1[] { const ids = new Set(nodes.map((node) => node.id)); return edges.filter((edge) => ids.has(edge.from) && ids.has(edge.to)).sort((left, right) => left.id.localeCompare(right.id)); }
...
function node(kind: GraphNodeV1["kind"], nodePath: string, name: string, symbolKind?: string, startLine?: number, endLine?: number, scope?: string): GraphNodeV1 { const discriminator = startLine === undefined ? undefined : scope === undefined || scope.length === 0 ? String(startLine) : `${scope}\u0000${startLine.toString()}`; return { id: createGraphNodeId(kind, nodePath, name, symbolKind, discriminator), kind, path: nodePath, name, ...(scope === undefined || scope.length === 0 ? {} : { scope }), ...(symbolKind === undefined ? {} : { symbolKind }), ...(startLine === undefined ? {} : { startLine }), ...(endLine === undefined ? {} : { endLine }) }; }
function resolveImport(from: string, specifier: string, files: ReadonlyMap<string, GraphNodeV1>): string | undefined { const base = specifier.startsWith(".") ? path.posix.normalize(path.posix.join(path.posix.dirname(from), specifier)) : specifier; const candidates = [base, ...[".ts", ".tsx", ".js", ".jsx", ".py", ".go", ".rs"].map((extension) => `${base}${extension}`), ...["index.ts", "index.js", "__init__.py"].map((index) => `${base}/${index}`)]; return candidates.find((candidate) => files.has(candidate)); }
function changedBuildPaths(previous: CodeGraphV1 | undefined, graph: CodeGraphV1): string[] { if (previous === undefined) return graph.files.map((file) => file.path).sort((left, right) => left.localeCompare(right)); const previousFiles = new Map(previous.files.map((file) => [file.path, file.contentHash])); const currentFiles = new Map(graph.files.map((file) => [file.path, file.contentHash])); return [...new Set([...previousFiles.keys(), ...currentFiles.keys()].filter((filePath) => previousFiles.get(filePath) !== currentFiles.get(filePath)))].sort((left, right) => left.localeCompare(right)); }
function boundedPaths(paths: readonly string[], limit: number | undefined): { values: string[]; truncated: boolean } { const max = entityLimit(limit); return { values: [...paths].sort((left, right) => left.localeCompare(right)).slice(0, max), truncated: paths.length > max }; }
  ...
  try {
    if (selection.cypher === undefined) {
      throw new MemexError(
        ...
        `Cypher requires the LadybugDB backend but the active tier is "${selection.tier}" (${selection.reason}). `
