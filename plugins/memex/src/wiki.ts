...
import {
  inspectInstructionBlocks,
  syncInstructionBlocks,
...
export interface WikiCheckIssue {
  code:
    ...
    | "INVALID_STATE"
    | "MALFORMED_INSTRUCTION_BLOCK"
    | "MISSING_INSTRUCTION_BLOCK"
    | "MISSING_PAGE"
    | "MISSING_PAGE_EDGE"
    | "MISSING_PAGE_NODE"
    | "STALE_GRAPH_REPORT"
    | "STALE_INSTRUCTION_BLOCK"
    | "STALE_STATE"
    | "SYMLINK";
  message: string;
  page?: string;
    ...
    for (const page of REQUIRED_WIKI_PAGES) {
      ...
      try {
        ...
        throw new MemexError(
          "IO_FAILURE",
          "Required Memex template could not be read.",
        );
      }
...
function instructionIssue(
  file: string,
  status: "fresh" | "missing" | "duplicate" | "malformed" | "stale",
): WikiCheckIssue | undefined {
  switch (status) {
    case "fresh":
      return undefined;
    case "missing":
