 ...
 */
export async function resolveRepositoryScope(root: string): Promise<string> {
  try {
...
function canonicalRepositoryRemote(input: string): string | undefined {
  const remote = input.trim();
  if (remote.length === 0 || remote.includes("\0") || remote.startsWith("/") || remote.startsWith("./") || remote.startsWith("../") || /^[A-Za-z]:[\\/]/u.test(remote)) return undefined;

  if (/^[A-Za-z][A-Za-z0-9+.-]*:\/\//u.test(remote)) {
    try {
      const url = new URL(remote);
      if (!["git:", "http:", "https:", "ssh:"].includes(url.protocol) || url.hostname.length === 0) return undefined;
  ...
  if (host === undefined || remotePath === undefined) return undefined;
...
function canonicalRemoteHost(hostname: string, port: string): string {
...
function canonicalRemotePath(value: string): string | undefined {
  ...
  if (segments.some((segment) => segment.length === 0 || segment === "." || segment === ".." || segment.includes("\0"))) return undefined;
