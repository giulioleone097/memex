...
export async function enumerateRepositoryMetadata(root: string, limits: { maxFiles: number; maxFileBytes: number; maxRepositoryBytes: number }): Promise<RepositoryEnumeration> {
  ...
  if (paths.length > limits.maxFiles) throw new MemexError("SOURCE_TOO_LARGE", "Repository exceeds the graph file limit.");
  let total = 0;
  ...
  for (const relative of paths) {
    ...
    if (details.isSymbolicLink()) throw new MemexError("SYMLINK_ESCAPE", "Graph scanner refuses symbolic-link repository files.");
    if (!details.isFile()) {
    ...
    if (details.size > limits.maxFileBytes) throw new MemexError("SOURCE_TOO_LARGE", "A repository file exceeds the graph file size limit.");
    total += details.size;
    if (total > limits.maxRepositoryBytes) throw new MemexError("SOURCE_TOO_LARGE", "Repository exceeds the graph byte limit.");
...
export async function readRepositoryFile(root: string, file: RepositoryFileMetadata): Promise<{ path: string; content: string; size: number; contentHash: string; language: string; sourceId: string }> {
  if (!safeRelativePath(file.path)) throw new MemexError("INVALID_ARGUMENT", "Graph repository path is invalid.");
  const absolute = path.resolve(root, file.path);
  assertInside(root, absolute);
  const details = await lstat(absolute);
  if (details.isSymbolicLink() || !details.isFile()) throw new MemexError("SYMLINK_ESCAPE", "Graph scanner refuses symbolic-link repository files.");
  const body = await readFile(absolute);
  if (body.includes(0)) throw new MemexError("UNSUPPORTED_SOURCE", "Graph scanner refuses binary repository files.");
...
export async function enumerateRepositoryFiles(root: string, limits: { maxFiles: number; maxFileBytes: number; maxRepositoryBytes: number }): Promise<Array<{ path: string; content: string; size: number; contentHash: string; language: string }>> {
  ...
  for (const file of files) {
    try { const loaded = await readRepositoryFile(root, file); results.push(loaded); } catch (error) { if (error instanceof MemexError && error.code === "UNSUPPORTED_SOURCE") continue; throw error; }
  }
  return results;
}
...
export async function readManifest(storage: GraphStorage): Promise<GraphManifest> {
  ...
  if (first === undefined) throw new MemexError("NOT_INITIALIZED", "No recoverable Memex graph manifest exists.");
...
export async function changedRepositoryPaths(root: string, base?: string): Promise<string[]> {
  if (base !== undefined) {
    if (!/^[a-f0-9]{7,64}$/iu.test(base)) throw new MemexError("INVALID_ARGUMENT", "Graph base must be a Git commit hash.");
...
export async function writeGraphUnlocked(storage: GraphStorage, graph: CodeGraphV1, shards: readonly GraphShard[], enrichmentShards: readonly EnrichmentShardV1[]): Promise<{ manifestPath: string; reusedShardCount: number }> {
  ...
  if (!published) {
    ...
    if (existing.generation !== generation) throw new MemexError("INVALID_STATE", "Graph generation publication changed during write.");
...
function parseManifest(value: unknown): GraphManifest {
  ...
  if (JSON.stringify(shards) !== JSON.stringify(value.shards)) throw new MemexError("INVALID_STATE", "Graph manifest shards are not canonical.");
  ...
  if (JSON.stringify(enrichmentShards) !== JSON.stringify(enrichmentShardsInput)) throw new MemexError("INVALID_STATE", "Graph manifest enrichment shards are not canonical.");
...
function parseManifestEnrichmentShard(value: unknown): { sourcePath: string; sourceContentHash: string; shard: string } {
  if (!isRecord(value) || !safeRelativePath(value.sourcePath) || typeof value.sourceContentHash !== "string" || !/^[a-f0-9]{64}$/iu.test(value.sourceContentHash) || !safeStoredName(value.shard)) throw new MemexError("INVALID_STATE", "Graph manifest enrichment shard is invalid.");
...
function parseManifestIndex(value: unknown, generation: string): GraphIndexManifest {
  ...
  const buckets = (entry: unknown): string[] => {
    if (!Array.isArray(entry)) throw new MemexError("INVALID_STATE", "Graph manifest index buckets are invalid.");
    const names: string[] = [];
    for (const name of entry) { if (typeof name !== "string" || !/^[a-f0-9]$/u.test(name)) throw new MemexError("INVALID_STATE", "Graph manifest index buckets are invalid."); names.push(name); }
    const ordered = names.slice().sort((left, right) => left.localeCompare(right));
    if (new Set(ordered).size !== ordered.length || JSON.stringify(entry) !== JSON.stringify(ordered)) throw new MemexError("INVALID_STATE", "Graph manifest index buckets are not canonical.");
    return ordered;
  };
...
function parseManifestShard(value: unknown): { path: string; contentHash: string; sourceId: string; shard: string } {
  if (!isRecord(value) || !safeRelativePath(value.path) || typeof value.contentHash !== "string" || !/^[a-f0-9]{64}$/iu.test(value.contentHash) || typeof value.sourceId !== "string" || value.sourceId.length === 0 || !safeStoredName(value.shard)) throw new MemexError("INVALID_STATE", "Graph manifest shard is invalid.");
...
function manifestGenerationPath(storage: GraphStorage, generation: string): string {
  if (!safeGeneration(generation)) throw new MemexError("INVALID_STATE", "Graph generation is invalid.");
...
function parseShard(value: unknown): GraphShard {
  ...
  if (symbols === undefined || relations === undefined || diagnostics === undefined) throw new MemexError("INVALID_STATE", "Graph shard is invalid.");
...
function parseShardRelation(value: unknown): NonNullable<SourceScan["relations"]>[number] {
  if (!isRecord(value) || !isRelationKind(value.kind) || typeof value.fromQualifiedName !== "string" || typeof value.target !== "string" || !positiveLine(value.line) || (value.confidence !== "resolved" && value.confidence !== "heuristic")) throw new MemexError("INVALID_STATE", "Graph shard relation is invalid.");
...
function parseShardDiagnostic(value: unknown): SourceScan["diagnostics"][number] {
  if (!isRecord(value) || !safeRelativePath(value.path) || typeof value.code !== "string" || typeof value.message !== "string") throw new MemexError("INVALID_STATE", "Graph shard diagnostic is invalid.");
...
function assertInside(root: string, candidate: string): void { const relative = path.relative(root, candidate); if (relative === "" || (!relative.startsWith(`..${path.sep}`) && relative !== ".." && !path.isAbsolute(relative))) return; throw new MemexError("PATH_OUTSIDE_ROOT", "Graph file path escapes the repository root."); }
async function assertRegularDirectory(directory: string): Promise<void> { const details = await lstat(directory); if (details.isSymbolicLink() || !details.isDirectory()) throw new MemexError("SYMLINK_ESCAPE", "Graph storage directory must not be a symbolic link."); }
async function isRegularFile(file: string): Promise<boolean> { try { const details = await lstat(file); if (details.isSymbolicLink()) throw new MemexError("SYMLINK_ESCAPE", "Graph manifest must not be a symbolic link."); return details.isFile(); } catch (error) { if (error instanceof MemexError) throw error; if (isNotFound(error)) return false; throw new MemexError("IO_FAILURE", "Unable to probe graph storage."); } }
function runGit(cwd: string, args: readonly string[]): Promise<string> { return new Promise((resolve, reject) => { const child = spawn("git", [...args], { cwd, shell: false, stdio: ["ignore", "pipe", "ignore"], windowsHide: true }); const chunks: Buffer[] = []; let settled = false; const fail = (): void => { if (!settled) { settled = true; child.kill("SIGKILL"); reject(new MemexError("GIT_FAILURE", "Unable to enumerate repository files for the graph.")); } }; const timeout = setTimeout(fail, 15_000); child.stdout.on("data", (chunk: Buffer) => { if (Buffer.concat(chunks).byteLength + chunk.byteLength > 16 * 1024 * 1024) { fail(); return; } chunks.push(chunk); }); child.once("error", fail); child.once("close", (code: number | null) => { clearTimeout(timeout); if (settled) return; settled = true; if (code !== 0) { reject(new MemexError("GIT_FAILURE", "Unable to enumerate repository files for the graph.")); return; } resolve(Buffer.concat(chunks).toString("utf8")); }); }); }
