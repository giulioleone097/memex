...
function parseCli(argv: readonly string[]): { operation: MemexOperation; flags: ParsedFlags } {
  const operation = argv[0];
  if (!isOperation(operation)) throw invalid("Memex operation is required and must be recognized.");
...
async function toRequest(operation: MemexOperation, flags: ParsedFlags, stdinText: string): Promise<{ operation: MemexOperation; input: Record<string, unknown> }> {
...
async function readIngestTransport(flags: ParsedFlags, stdinText: string): Promise<string> {
  ...
  if (file === undefined) return enforceEnvelopeByteLimit(stdinText);
  if (file === true) throw invalid("Input file requires a path.");
...
function enforceEnvelopeByteLimit(value: string): string {
  if (Buffer.byteLength(value, "utf8") > MAX_ENVELOPE_BYTES) throw sourceEnvelopeTooLarge();
  return value;
}

async function readBoundedEnvelopeFile(filePath: string): Promise<string> {
  try {
    const handle = await open(filePath, fsConstants.O_RDONLY | fsConstants.O_NOFOLLOW);
    try {
      const stat = await handle.stat();
      if (!stat.isFile()) throw new MemexError("NOT_FOUND", "Input file could not be read.");
      if (stat.size > MAX_ENVELOPE_BYTES) throw sourceEnvelopeTooLarge();
...
function sourceEnvelopeTooLarge(): MemexError {
...
function emitFailure(error: unknown, pretty: boolean): number {
  const result = error instanceof MemexError
    ...
    : { ok: false as const, error: { code: "IO_FAILURE", message: "Memex operation failed unexpectedly." } };
  output.write(`${JSON.stringify(result, null, pretty ? 2 : undefined)}\n`);
