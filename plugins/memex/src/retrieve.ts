...
export async function search(request: SearchRequest, ports: RetrievalPorts): Promise<SearchResultV1> {
  ...
  if (signals.includes("vector")) {
    if (ports.embedder === undefined || ports.vectorStore === undefined) throw new MemexError("MODEL_ASSET_MISSING", "Vector retrieval requires a loaded embedder and vector store.");
    const status = await ports.vectorStore.status();
    if (!status.compatible) throw new MemexError("INDEX_INCOMPATIBLE", "Vector store model does not match the loaded embedder.");
  ...
  if (signals.includes("graph")) {
    if (ports.graphIndex === undefined) throw new MemexError("NOT_INITIALIZED", "Graph retrieval requires a built graph index.");
...
export function validateSignals(signals: SearchRequest["signals"]): SignalName[] {
  ...
  if (signals.length === 0) throw new MemexError("INVALID_ARGUMENT", "Signals must include at least one of lexical, vector, graph.");
  const unique = new Set(signals);
  for (const signal of unique) if (!ALL_SIGNALS.includes(signal)) throw new MemexError("INVALID_ARGUMENT", `Unknown retrieval signal: ${signal}.`);
