 ...
 */
export async function runMigration(homeDir: string): Promise<MigrationResultV1> {
  ...
  if (legacyStatus.isSymbolicLink()) {
    throw new MemexError(
      "SYMLINK_ESCAPE",
      "Legacy storage root must not be a symbolic link.",
    );
  }
  ...
  if (newStatus !== null) {
    if (newStatus.isSymbolicLink()) {
      throw new MemexError(
        "SYMLINK_ESCAPE",
        "New storage root must not be a symbolic link.",
      );
    }
    ...
    if ((await readdir(to)).length > 0) {
      throw new MemexError(
        "MIGRATION_CONFLICT",
        "Both the legacy ~/.openwiki root and the new ~/.memex root contain data; resolve manually before migrating.",
      );
    }
    targetExists = true;
  }
...
async function moveDirectory(from: string, to: string, targetExists: boolean): Promise<void> {
  if (targetExists) {
  ...
  try {
    ...
    if (!isCrossDeviceError(error)) {
      throw new MemexError(
        "IO_FAILURE",
        "Unable to move legacy storage to the new root.",
      );
    }
  }
  ...
  try {
    ...
    throw new MemexError(
      "IO_FAILURE",
      "Unable to copy legacy storage to the new root.",
    );
  }
  ...
  if (sourceCount !== destinationCount) {
    throw new MemexError(
      "IO_FAILURE",
      "Legacy storage copy could not be verified; the original was left untouched.",
    );
  }
...
async function isTombstonedOnly(from: string): Promise<boolean> {
  const entries = await readdir(from);
  return entries.length === 1 && entries[0] === TOMBSTONE_FILE_NAME;
}

async function writeTombstone(tombstonePath: string, newRoot: string): Promise<void> {
