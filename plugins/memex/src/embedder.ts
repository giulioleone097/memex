...
async function verifyVendorEntry(vendorRoot: string, entry: VendorManifestEntry): Promise<void> {
  if (entry.parts === undefined) {
    if (entry.sha256 === undefined) throw new MemexError("MODEL_ASSET_CORRUPT", `Vendor asset manifest entry for ${entry.path} declares neither sha256 nor parts.`);
...
async function verifyVendorFile(vendorRoot: string, relativePath: string, expectedSha256: string, expectedBytes: number): Promise<void> {
  ...
  if (size !== expectedBytes) throw new MemexError("MODEL_ASSET_CORRUPT", `Vendor asset size mismatch: ${relativePath}.`);
  const digest = await sha256File(absolute);
  if (digest !== expectedSha256.toLowerCase()) throw new MemexError("MODEL_ASSET_CORRUPT", `Vendor asset checksum mismatch: ${relativePath}.`);
}
...
export async function loadModelBuffer(vendorRoot: string, manifest: VendorManifest, logicalPath: string): Promise<Uint8Array> {
  ...
  if (entry === undefined) throw new MemexError("MODEL_ASSET_MISSING", `Vendor manifest has no entry for ${logicalPath}.`);
  if (entry.parts === undefined) {
    if (entry.sha256 === undefined) throw new MemexError("MODEL_ASSET_MISSING", `Vendor manifest entry for ${logicalPath} declares neither a single file nor parts.`);
  ...
  if (entry.assembledSha256 === undefined) throw new MemexError("MODEL_ASSET_CORRUPT", `Vendor manifest is missing the assembled checksum for ${logicalPath}.`);
  const actual = createHash("sha256").update(assembled).digest("hex");
  if (actual !== entry.assembledSha256.toLowerCase()) throw new MemexError("MODEL_ASSET_CORRUPT", `Assembled checksum mismatch for ${logicalPath}.`);
  return assembled;
}
...
async function loadOrtRuntime(vendorRoot: string): Promise<OrtRuntime> {
  ...
  return {
    async createSession(modelPathOrBuffer) {
      ...
      if (typeof record.run !== "function") throw new MemexError("MODEL_ASSET_CORRUPT", "ONNX Runtime session is missing run().");
    ...
    createTensor(type, data, dims) {
      ...
      return constructed as OrtTensorLike;
    },
  };
}
...
async function runInference(runtime: OrtRuntime, session: OrtSessionLike, tokenizer: Tokenizer, text: string): Promise<Float32Array> {
  ...
  if (ids.length === 0) throw new MemexError("EMBEDDING_FAILURE", "Tokenizer produced an empty sequence.");
  ...
  if (hiddenTensor === undefined) throw new MemexError("EMBEDDING_FAILURE", "ONNX Runtime returned no output tensor.");
  const [batch, tensorSeqLen, dims] = hiddenTensor.dims;
  if (batch !== 1 || tensorSeqLen !== seqLen || dims !== DIMS) throw new MemexError("EMBEDDING_FAILURE", "ONNX Runtime output tensor has an unexpected shape.");
...
function parseManifest(value: unknown): VendorManifest {
  if (!isRecord(value) || !Array.isArray(value.assets)) throw new MemexError("MODEL_ASSET_CORRUPT", "Vendor asset manifest is invalid.");
...
function parseManifestEntry(value: unknown): VendorManifestEntry {
  ...
  if (hasSingleHash) {
    ...
    if (typeof sha256 !== "string" || !/^[a-f0-9]{64}$/iu.test(sha256)) throw new MemexError("MODEL_ASSET_CORRUPT", `Vendor asset manifest entry for ${value.path} has an invalid sha256.`);
    return { ...base, sha256 };
  }
  const assembledSha256 = (value as { assembled_sha256?: unknown }).assembled_sha256;
  if (typeof assembledSha256 !== "string" || !/^[a-f0-9]{64}$/iu.test(assembledSha256)) throw new MemexError("MODEL_ASSET_CORRUPT", `Vendor asset manifest entry for ${value.path} has an invalid assembled_sha256.`);
  ...
  const parts = (value.parts as unknown[]).map((part) => parseManifestPart(part, entryPath));
  parts.forEach((part, index) => {
    if (!part.path.endsWith(`.part${String(index)}`)) throw new MemexError("MODEL_ASSET_CORRUPT", `Vendor asset manifest parts for ${entryPath} must be declared in order part0, part1, ....`);
  });
...
function asRecord(value: unknown): Record<string, unknown> {
  if (!isRecord(value)) throw new MemexError("MODEL_ASSET_CORRUPT", "Vendored ONNX Runtime Web module returned an invalid value.");
  return value;
}

function asTensor(value: unknown): OrtTensorLike {
  const record = asRecord(value);
  if (!Array.isArray(record.dims) || record.data === undefined || record.data === null) throw new MemexError("MODEL_ASSET_CORRUPT", "ONNX Runtime Tensor has an unexpected shape.");
...
// for feed tensors.
function assertTensorShape(value: unknown): void {
  const record = asRecord(value);
  if (!Array.isArray(record.dims) || record.data === undefined || record.data === null) throw new MemexError("MODEL_ASSET_CORRUPT", "ONNX Runtime Tensor has an unexpected shape.");
}
...
function isCallableRecord(value: unknown): value is Record<string, unknown> {
