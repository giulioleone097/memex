...
import { hostHomeDir, resolveWikiLocation, type WikiLocation } from "./paths.js";
import {
  ask as retrieveAsk,
...
export const GRAPH_ACTIONS: readonly GraphAction[] = [
  "build",
  "status",
  "query",
  "context",
...
const CHECK_PHASES = ["preflight", "strict"] as const satisfies readonly WikiCheckPhase[];
...
export async function dispatch(request: DispatchRequest): Promise<MemexJsonResult<unknown>> {
  try {
...
export function toEnvelope(error: unknown): MemexJsonFailure {
  if (error instanceof MemexError) return { ok: false, error: error.toJSON() };
  return {
    ok: false,
    error: { code: "IO_FAILURE", message: "Memex operation failed unexpectedly." },
  };
}
...
async function dispatchUnsafe(request: DispatchRequest): Promise<unknown> {
  ...
  switch (request.operation) {
    case "init": {
      ...
      return {
        ...
        location: {
          ...
          dataRoot: path.join(hostHomeDir(), ".memex", "data", initialized.location.workspaceId),
          ...(locationOptions.root === undefined
            ? {}
            : { workspaceRoot: locationOptions.root, wikiRoot: path.join(locationOptions.root, "memex") }),
        },
      };
    }
    case "status": {
    ...
    case "doctor": {
      const location = await resolveWikiLocation(readLocation(input, ["mode", "root"]));
      return runDoctor({ location, homeDir: hostHomeDir() });
    }
    case "schedule":
    ...
    case "migrate":
      assertKeys(input, []);
      return runMigration(hostHomeDir());
  }
}
...
function readAsyncFunction(record: InputRecord, key: string): (options: InputRecord) => Promise<unknown> {
  ...
  if (typeof value !== "function") throw new MemexError("IO_FAILURE", "Native graph module is unavailable.");
