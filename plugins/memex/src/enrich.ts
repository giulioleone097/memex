...
export async function enrichGraph(options: EnrichOptions): Promise<EnrichResult> {
  ...
  return withGraphWriteLock(resolved.storage, async () => {
    ...
    for (const edge of redacted.edges) {
      if (!validEdgeConfidence(edge.kind, edge.confidence)) throw new MemexError("INVALID_ARGUMENT", "Enrich edge confidence is not valid for its edge kind.");
...
async function assertContentHashMatches(repositoryRoot: string, sourcePath: string, expectedHash: string): Promise<void> {
  ...
  if (details.isSymbolicLink() || !details.isFile()) throw new MemexError("SYMLINK_ESCAPE", "Enrich sourcePath must be a regular repository file.");
  const content = await readFile(absolute);
  const actualHash = createHash("sha256").update(content).digest("hex");
  if (actualHash !== expectedHash) throw new MemexError("INVALID_ARGUMENT", "Enrich sourceContentHash does not match the current file content.");
}
