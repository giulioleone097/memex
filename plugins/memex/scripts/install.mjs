...
async function runClient(client, argv, commands) {
  ...
  const outputRoot = mkdtempSync(join(tmpdir(), "memex client output "));
...
export async function pollInstalledRuntime(
  ...
  throw new LifecycleError(
    "INSTALL_NOT_READY",
    "The installed Memex runtime did not become ready in time.",
...
async function waitForInstalledRuntime(client, argv, commands) {
  try {
    ...
    throw new LifecycleError(
      error.code,
      `${client} did not expose a complete installed Memex runtime in time.`,
