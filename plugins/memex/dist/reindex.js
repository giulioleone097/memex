...
import { chunkMarkdown, chunkSymbols, symbolChunkText } from "./chunk.js";
...
export async function reindexWikiPage(location, page, content, ports = {}) {
...
export async function reindexCodeSymbols(root, index, homeDir, ports = {}) {
    ...
    const refs = chunkSymbols(symbols).map((ref) => {
        ...
        if (node === undefined)
            throw new MemexError("INVALID_STATE", "Chunked symbol node disappeared during reindexing.");
        return { ref, text: symbolChunkText(node) };
    });
    return reindexChunks(location.dataRoot, refs, ports);
}
async function reindexChunks(dataRoot, refs, ports) {
    if (refs.length === 0)
        return { chunked: 0, embedded: 0, reusedVectors: 0, reusedLexical: 0, embeddingsAvailable: true };
    ...
    if (changed.length === 0)
        return { chunked: refs.length, embedded: 0, reusedVectors: refs.length, reusedLexical: refs.length, embeddingsAvailable: true };
    ...
    try {
        ...
        if (error instanceof MemexError && (error.code === "MODEL_ASSET_MISSING" || error.code === "MODEL_ASSET_CORRUPT")) {
    ...
    const entries = changed.map(({ ref }, index) => {
        ...
        if (vector === undefined)
            throw new MemexError("EMBEDDING_FAILURE", "Embedder returned fewer vectors than requested.");
        return { ref, vector };
    });
