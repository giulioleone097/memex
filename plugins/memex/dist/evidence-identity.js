 ...
 */
export function createEvidenceIdentity(input) {
    ...
    if (endLine < startLine)
        throw new TypeError("Evidence endLine must be greater than or equal to startLine.");
...
function normalizeProjectScope(value) {
    ...
    if (isAbsolutePath(normalized))
        throw new TypeError("Evidence projectScope must be a logical identity, not an absolute host path.");
...
function normalizeSourceIdentity(value) {
    ...
    if (isAbsolutePath(normalized))
        return `source-sha256:${sha256(normalized)}`;
    return normalized.replace(/^\.\//u, "");
}
...
function normalizeBoundary(value, field) {
    if (!Number.isSafeInteger(value) || value < 1)
        throw new TypeError(`Evidence ${field} must be a positive integer.`);
    return value;
}
function normalizePriorEvidenceId(value) {
    const normalized = nonEmpty(value, "priorEvidenceId");
    if (!/^ev1:[a-f0-9]{64}$/u.test(normalized))
        throw new TypeError("Evidence priorEvidenceId must be an ev1 SHA-256 identity.");
    return normalized;
}
function nonEmpty(value, field) {
    ...
    if (value.includes("\0"))
        throw new TypeError(`Evidence ${field} must not contain NUL bytes.`);
    return value.trim();
}
