...
// or writes.
export function legacyStorageRoot(homeDir) {
...
export function hostHomeDir() {
    return process.env.HOME ?? process.env.USERPROFILE ?? os.homedir();
}
export async function resolveWikiLocation(options) {
    if (options.mode === "code") {
        if (!options.root) {
            throw new MemexError("INVALID_ARGUMENT", "Code mode requires a repository root.");
        }
        ...
        return {
            ...
            dataRoot: await resolveProspectivePath(homeDir, path.join(".memex", "data", workspaceId)),
        };
    }
    const homeDir = await resolveExistingDirectory(options.homeDir ?? os.homedir());
    const privateRoot = await resolveProspectivePath(homeDir, ".memex");
    const wikiRoot = await resolveProspectivePath(privateRoot, "wiki");
    const dataRoot = await resolveProspectivePath(homeDir, path.join(".memex", "data", "personal"));
    return {
        mode: "personal",
...
export async function resolveConfinedMarkdownPath(location, page) {
...
function validatePagePath(page) {
    if (page.length === 0 ||
        page.includes("\0") ||
        page.includes("\\") ||
        path.isAbsolute(page) ||
        path.win32.isAbsolute(page)) {
        throw new MemexError("PATH_OUTSIDE_ROOT", "Wiki page path must be relative to the wiki root.");
    }
    const segments = page.split("/");
    if (segments.some((segment) => segment.length === 0 || segment === "." || segment === "..")) {
        throw new MemexError("PATH_OUTSIDE_ROOT", "Wiki page path must stay inside the wiki root.");
    }
    if (!page.endsWith(".md")) {
        throw new MemexError("INVALID_ARGUMENT", "Wiki pages must use the .md extension.");
    }
}
...
async function resolveProspectivePath(canonicalRoot, relativePath) {
...
function assertInside(root, candidate) {
    ...
    throw new MemexError("SYMLINK_ESCAPE", "Resolved path escapes the allowed wiki root.");
}
