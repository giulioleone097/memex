...
async function main() {
    ...
    if (payload === undefined || payload.hook_event_name !== "SessionStart" || typeof payload.cwd !== "string")
        return;
...
async function nearestGitRoot(cwd) {
    try {
...
async function readSnapshot(mode, root) {
    try {
        const location = await resolveWikiLocation({ mode, ...(root === undefined ? {} : { root }) });
        const state = await readState(location);
        if (!isValidSnapshot(location, state.mode, state.wikiRoot, state.updatedAt))
            return undefined;
...
function isValidSnapshot(location, mode, wikiRoot, updatedAt) {
...
function renderContext(snapshot) {
    ...
    const prefix = `Memex mode=${snapshot.mode}; wikiRoot=`;
