...
import { hostHomeDir } from "./paths.js";
...
export const RETRIEVAL_HEALTH_SCHEMA = "memex.retrieval-health.v1";
/**
 ...
 */
export async function readRetrievalHealth(location, options = {}) {
    ...
    const failure = (category, code, message, input, output = {}, signal) => {
        const value = { code, message, input, output };
        failuresByCategory[category].push(value);
        if (signal !== undefined)
            failuresBySignal[signal].push(value);
        return value;
    };
    let state = null;
    let stateError;
    ...
    if (repositoryIdentity === null)
        failure("source", "REPOSITORY_IDENTITY_UNAVAILABLE", "Stable repository identity could not be established.", sourceInput);
    else if (repositoryIdentity === UNSCOPED_PROJECT_SCOPE)
        failure("source", "REPOSITORY_IDENTITY_UNSCOPED", "Repository has no portable origin identity.", sourceInput);
    if (state === null && stateError === undefined)
        failure("source", "STATE_MISSING", "Wiki state is not initialized.", sourceInput);
    let lexicalCount = 0;
    let vectorCount = 0;
    let vectorModelId;
    let vectorModelRevision;
    let vectorAvailable = true;
    let vectorUnavailableReason;
    let lexicalManifest;
    let vectorManifest;
    ...
    try {
        ...
        if (lexicalCount > 0 && probeTerm === undefined)
            throw new Error("Lexical index has chunks but no searchable terms.");
        if (probeTerm !== undefined && (await lexicalIndex.search(probeTerm, 1)).length === 0)
            throw new Error("Lexical index returned no result for an indexed term.");
    ...
    try {
        ...
        if (vectorModelId === undefined || vectorModelRevision === undefined || vectorDims === undefined)
            throw new Error("Vector index model metadata is invalid.");
        ...
        if (!vectorStatus.compatible || vectorStatus.chunks !== vectorCount)
            throw new Error("Vector index status does not match its manifest.");
        if (vectorCount > 0 && (await vectorStore.search(new Float32Array(vectorDims), 1)).length === 0)
            throw new Error("Vector index has chunks but no readable vectors.");
        ...
        if (!vectorAvailable)
            failure("installedCache", "VECTOR_EMBEDDINGS_UNAVAILABLE", vectorUnavailableReason ?? "Vector embeddings are unavailable.", { path: vectorManifestPath, signal: "vector" }, { embeddingsAvailable: false }, "vector");
    ...
    if (location.mode === "code" && location.workspaceRoot !== undefined) {
        try {
            ...
            if (!probed.initialized)
                throw new Error("Graph index is not initialized.");
    ...
    try {
        ...
        if (state === null)
            failure("hostMount", "STATE_MOUNT_UNAVAILABLE", "Resolved host state path is not readable.", hostInput, { error: safeError(error) });
    }
    ...
    if (!lexicalValidated || lexicalCount === 0)
        failure("liveCall", "LEXICAL_LIVE_UNAVAILABLE", "Lexical retrieval has no validated indexed chunks.", { signal: "lexical", indexed: lexicalCount }, { validated: lexicalValidated }, "lexical");
    if (!vectorValidated || vectorCount === 0)
        failure("liveCall", "VECTOR_LIVE_UNAVAILABLE", "Vector retrieval is not ready.", { signal: "vector", indexed: vectorCount }, { validated: vectorValidated, embeddingsAvailable: vectorAvailable, unavailableReason: vectorUnavailableReason }, "vector");
    if (location.mode === "code" && !graphAvailable)
        failure("liveCall", "GRAPH_LIVE_UNAVAILABLE", "Graph retrieval has no recoverable generation.", { signal: "graph" }, {}, "graph");
    ...
    const reasons = [];
    if (expected === 0)
        reasons.push("No expected retrieval chunks are present.");
    if (indexed === 0)
        reasons.push("No retrieval chunks are indexed across lexical and vector stores.");
    if (!identity.matches)
        reasons.push("Source identity does not match the resolved workspace.");
    if (fresh.status !== "current")
        reasons.push(fresh.reason ?? "Retrieval freshness is not current.");
    if (requiredFailures.length > 0)
        reasons.push("One or more required retrieval proofs failed.");
...
async function readFreshness(location, stateAvailable, indexedAt, graphAvailable, fail) {
    if (!stateAvailable)
        return { status: "unknown", ...(indexedAt === undefined ? {} : { indexedAt }), reason: "Wiki state is unavailable, so source freshness cannot be established." };
    try {
        ...
        if (location.mode === "code" && !graphAvailable)
            return { status: "unknown", ...(indexedAt === undefined ? {} : { indexedAt }), reason: "No validated graph generation is available to establish repository freshness." };
        if (location.mode === "code" && location.workspaceRoot !== undefined) {
            const status = await getGraphStatus({ root: location.workspaceRoot, homeDir: hostHomeDir() });
            if (!status.available)
                return { status: "unknown", ...(indexedAt === undefined ? {} : { indexedAt }), reason: status.reason ?? "Graph status is unavailable." };
            if (!status.fresh) {
...
function recoveryPlan(reasons, location, vectorAvailable) {
    if (reasons.length === 0)
        return { required: false, action: "No recovery required.", commands: [] };
    const commands = [];
    if (location.mode === "code")
        commands.push(`memex graph --action build --root ${location.workspaceRoot ?? "<root>"}`);
    if (!vectorAvailable)
        commands.push("memex doctor");
...
async function readManifestRecord(filePath) {
...
function countChunks(manifest) {
    if (Array.isArray(manifest.chunks))
        return manifest.chunks.length;
...
function firstManifestTerm(manifest) {
    if (manifest.documentFrequency === null || typeof manifest.documentFrequency !== "object" || Array.isArray(manifest.documentFrequency))
        return undefined;
