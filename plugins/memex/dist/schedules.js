...
export const SCHEDULE_COMMANDS = ["update", "ingest"];
...
export async function setSchedule(options) {
    ...
    return withWikiLock(options.location.dataRoot, async () => {
        ...
        const existingIndex = current.schedules.findIndex((candidate) => candidate.id === schedule.id);
...
function parseSchedule(input, code) {
    ...
    if (schedule.schemaVersion !== 1) {
        throw new MemexError(code, "Schedule intent schema version must be 1.");
    }
    if (!isScheduleCommand(schedule.command)) {
        throw new MemexError(code, "Schedule command must be update or ingest.");
    }
    ...
    if (typeof schedule.enabled !== "boolean") {
        throw new MemexError(code, "Schedule enabled must be a boolean.");
    }
    ...
    if (schedule.command === "ingest" && sourceId === undefined) {
        throw new MemexError(code, "Ingest schedules require a sourceId.");
    }
    if (schedule.command === "update" && sourceId !== undefined) {
        throw new MemexError(code, "Update schedules must not include a sourceId.");
    }
    const parsed = {
        schemaVersion: 1,
        id,
    ...
    if (containsSensitive(parsed)) {
        throw new MemexError(code, "Schedule intent must not contain credential-shaped values.");
    }
    return parsed;
}
function parseScheduleStore(input) {
    const store = requireRecord(input, "INVALID_STATE", "Schedule state must be an object.");
    ...
    if (store.schemaVersion !== 1 || !Array.isArray(store.schedules)) {
        throw new MemexError("INVALID_STATE", "Schedule state is invalid.");
    }
    const schedules = store.schedules.map((schedule) => parseSchedule(schedule, "INVALID_STATE"));
    const ids = new Set();
    for (const schedule of schedules) {
        if (ids.has(schedule.id)) {
            throw new MemexError("INVALID_STATE", "Schedule state contains duplicate ids.");
        }
        ids.add(schedule.id);
    }
    schedules.sort((left, right) => compareStrings(left.id, right.id));
...
async function readScheduleStore(location) {
    ...
    try {
        ...
        try {
            ...
            throw new MemexError("INVALID_STATE", "Schedule state is not valid JSON.");
        }
        ...
        if (isSymlinkError(error)) {
            throw new MemexError("SYMLINK_ESCAPE", "Schedule state must not be a symbolic link.");
        }
        if (error instanceof MemexError) {
            throw error;
        }
        throw new MemexError("IO_FAILURE", "Unable to read schedule state.");
    }
    finally {
        await handle?.close().catch(() => undefined);
    }
}
async function writeScheduleStore(location, store) {
    const validated = parseScheduleStore(store);
    await atomicWriteFile(path.join(location.dataRoot, SCHEDULES_FILE_NAME), `${JSON.stringify(validated, null, 2)}\n`);
}
async function ensureDataRoot(dataRoot) {
    const existing = await inspectPath(dataRoot);
    if (existing?.isSymbolicLink()) {
        throw new MemexError("SYMLINK_ESCAPE", "Schedule storage must not be a symbolic link.");
    }
    if (existing !== null && !existing.isDirectory()) {
        throw new MemexError("IO_FAILURE", "Schedule storage is invalid.");
    }
    if (existing !== null) {
        return;
    }
    try {
        ...
        throw new MemexError("IO_FAILURE", "Unable to create schedule storage.");
    }
}
async function dataRootExists(dataRoot) {
...
async function inspectPath(filePath) {
    try {
        ...
        throw new MemexError("IO_FAILURE", "Unable to inspect schedule storage.");
    }
}
function schedulesEqual(left, right) {
    return JSON.stringify(left) === JSON.stringify(right);
}
function parseScheduleId(value, code) {
    if (typeof value !== "string" || !SCHEDULE_ID_PATTERN.test(value)) {
        throw new MemexError(code, "Schedule id is invalid.");
    }
    return value;
}
function parseCron(value, code) {
    if (typeof value !== "string" || value.length > 256 || value.includes("\0")) {
        throw new MemexError(code, "Schedule cron expression is invalid.");
    }
...
function parseCronValue(value, spec) {
    ...
    const parsed = namedIndex === undefined || namedIndex === -1
...
function parseCronNumber(value) {
    if (!/^\d+$/u.test(value)) {
        return null;
    }
...
function parseTimezone(value, code) {
    if (typeof value !== "string" ||
        ...
        value.includes("\n") ||
        value.includes("\r")) {
        throw new MemexError(code, "Schedule timezone is invalid.");
    }
    try {
...
function parseSourceId(value, code) {
    if (typeof value !== "string" ||
        value.trim().length === 0 ||
        value.length > 512 ||
        value.includes("\0") ||
        value.includes("\n") ||
        value.includes("\r")) {
        throw new MemexError(code, "Schedule sourceId is invalid.");
    }
    return value;
}
...
function requireKnownKeys(value, knownKeys, code, label) {
    if (Object.keys(value).some((key) => !knownKeys.has(key))) {
        throw new MemexError(code, `${label} contains unsupported fields.`);
    }
}
