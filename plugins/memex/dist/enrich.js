...
import { createGraphEdgeId, createGraphNodeId, mergeEnrichment, validEdgeConfidence, } from "./graph-contracts.js";
import { assembleGraph } from "./graph.js";
import { openGraphIndex, readEnrichmentShard, readGraphShard, readManifest, resolveGraphStorage, withGraphWriteLock, writeGraphUnlocked, } from "./graph-store.js";
import { redactSensitive } from "./redact.js";
export async function enrichGraph(options) {
    ...
    return withGraphWriteLock(resolved.storage, async () => {
        ...
        for (const edge of redacted.edges) {
            if (!validEdgeConfidence(edge.kind, edge.confidence))
                throw new MemexError("INVALID_ARGUMENT", "Enrich edge confidence is not valid for its edge kind.");
        ...
        if (existing?.sourceContentHash === redacted.sourceContentHash) {
            return { action: "enrich", sourcePath: redacted.sourcePath, sourceContentHash: redacted.sourceContentHash, applied: false, nodesWritten: 0, edgesWritten: 0, nodeIds: Object.fromEntries(publicNodeIds), generatedAt: manifest.generatedAt };
        }
        const now = options.now ?? new Date().toISOString();
        const sourceNodeId = createGraphNodeId("source", redacted.sourcePath, redacted.sourceContentHash);
        const sourceNode = { id: sourceNodeId, kind: "source", path: redacted.sourcePath, name: redacted.sourceContentHash };
        const groundsEdges = resolvedNodes.map((node) => ({ id: createGraphEdgeId("grounds", sourceNodeId, node.id, "extracted"), kind: "grounds", from: sourceNodeId, to: node.id, confidence: "extracted" }));
        const shard = {
        ...
        const otherEnrichmentShards = await Promise.all(manifest.enrichmentShards.filter((entry) => entry.sourcePath !== redacted.sourcePath).map((entry) => readEnrichmentShard(resolved.storage, entry.shard)));
        ...
        return { action: "enrich", sourcePath: redacted.sourcePath, sourceContentHash: redacted.sourceContentHash, applied: true, nodesWritten: shard.nodes.length, edgesWritten: shard.edges.length, nodeIds: Object.fromEntries(publicNodeIds), generatedAt: now };
    });
}
async function resolveNodeRef(index, localNodeIds, ref) {
    const local = localNodeIds.get(ref);
    if (local !== undefined)
        return local;
    if (/^[a-f0-9]{64}$/u.test(ref) && (await index.node(ref)) !== undefined)
        return ref;
    throw new MemexError("INVALID_ARGUMENT", "Enrich edge references an unknown node.");
}
function localKey(kind, path, name) {
    return `${kind}:${path}:${name}`;
}
async function assertContentHashMatches(repositoryRoot, sourcePath, expectedHash) {
    const absolute = path.resolve(repositoryRoot, sourcePath);
    const relative = path.relative(repositoryRoot, absolute);
    if (relative === "" || relative.startsWith(`..${path.sep}`) || relative === ".." || path.isAbsolute(relative)) {
        throw new MemexError("PATH_OUTSIDE_ROOT", "Enrich sourcePath escapes the repository root.");
    }
    let details;
    try {
        ...
        throw new MemexError("NOT_FOUND", "Enrich sourcePath was not found.");
    }
    if (details.isSymbolicLink() || !details.isFile())
        throw new MemexError("SYMLINK_ESCAPE", "Enrich sourcePath must be a regular repository file.");
    const content = await readFile(absolute);
    const actualHash = createHash("sha256").update(content).digest("hex");
    if (actualHash !== expectedHash)
        throw new MemexError("INVALID_ARGUMENT", "Enrich sourceContentHash does not match the current file content.");
}
