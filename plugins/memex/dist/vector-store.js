...
export async function openVectorStore(storageRoot, model) {
    ...
    return {
        async upsert(entries) {
            return withFileWriteLock(lockPath, async () => {
                ...
                if (manifest.modelId !== model.modelId || manifest.dims !== model.dims)
                    throw new MemexError("INDEX_INCOMPATIBLE", "Vector store manifest does not match the active embedding model.");
                ...
                for (const entry of entries) {
                    ...
                    if (existing !== undefined)
                        touchedBuckets.add(existing.bucket);
                ...
                for (const bucket of touchedBuckets) {
                    ...
                    for (const entry of chunksById.values()) {
                        ...
                        if (preserved !== undefined)
                            rows.push({ ref: entry.ref, vector: preserved });
                    }
                ...
                for (const entry of chunks)
                    segmentCounts.set(entry.bucket, (segmentCounts.get(entry.bucket) ?? 0) + 1);
                const segments = [...segmentCounts.entries()].sort(([left], [right]) => left.localeCompare(right)).map(([bucket, count]) => {
                    ...
                    if (contentHash === undefined)
                        throw new MemexError("INVALID_STATE", `Vector segment ${bucket} is missing a content hash.`);
        ...
        async search(vector, limit) {
            if (!Number.isSafeInteger(limit) || limit < 1 || limit > 200)
                throw new MemexError("INVALID_ARGUMENT", "Vector search limit must be between 1 and 200.");
            const manifest = await readManifest();
            if (manifest === undefined || manifest.chunks.length === 0)
                return [];
            if (manifest.modelId !== model.modelId || manifest.dims !== model.dims)
                throw new MemexError("INDEX_INCOMPATIBLE", "Vector store manifest does not match the active embedding model.");
        ...
        async status() {
            ...
            if (manifest === undefined)
                return { modelId: model.modelId, dims: model.dims, chunks: 0, compatible: true, embeddingsAvailable: true };
            return {
...
export async function readVectorChunkDigest(storageRoot) {
    let manifest;
    try {
        manifest = parseManifest(JSON.parse(await readFile(path.join(storageRoot, "manifest.json"), "utf8")));
    }
    catch {
        return new Map();
    }
...
export async function markEmbeddingsUnavailable(storageRoot, model, reason) {
    await mkdir(storageRoot, { recursive: true, mode: 0o700 });
    const manifestPath = path.join(storageRoot, "manifest.json");
    const lockPath = path.join(storageRoot, "writer.lock");
    await withFileWriteLock(lockPath, async () => {
        ...
        const flagged = { ...manifest, embeddingsAvailable: false, unavailableReason: reason };
        await atomicWriteFile(manifestPath, `${JSON.stringify(flagged)}\n`);
    }, { waitMs: WRITE_LOCK_WAIT_MS, staleMs: STALE_LOCK_MS });
}
function emptyManifest(model) {
...
function quantize(vector) {
    ...
    for (let index = 0; index < vector.length; index += 1)
        output[index] = Math.max(-127, Math.min(127, Math.round((vector[index] ?? 0) * 127)));
    return output;
}
function dequantize(row) {
    const output = new Float32Array(row.length);
    for (let index = 0; index < row.length; index += 1)
        output[index] = (row[index] ?? 0) / 127;
    return output;
}
...
function bucketFor(contentHash) {
    ...
    if (first === undefined || !/^[0-9a-f]$/u.test(first))
        throw new MemexError("INVALID_STATE", "Chunk contentHash must be a lowercase hex hash.");
    return first;
}
function encodeId(id) {
    if (!/^[a-f0-9]{64}$/u.test(id))
        throw new MemexError("INVALID_STATE", "Chunk id must be a 64-character lowercase hex hash.");
    return Buffer.from(id, "ascii");
}
function encodeBucket(rows, dims) {
...
async function readBucketFile(storageRoot, bucket, dims) {
    let buffer;
    try {
...
async function readBucketRow(storageRoot, bucket, id, dims) {
    return (await readBucketFile(storageRoot, bucket, dims)).find((row) => row.id === id)?.vector;
}
function decodeBucket(buffer, dims) {
    if (buffer.length < HEADER_BYTES || buffer.toString("ascii", 0, 4) !== SEGMENT_MAGIC)
        throw new MemexError("MODEL_ASSET_CORRUPT", "Vector segment header is invalid.");
    const dtype = buffer.readUInt8(4);
    const storedDims = buffer.readUInt16LE(5);
    const count = buffer.readUInt32LE(7);
    if (dtype !== 1 || storedDims !== dims)
        throw new MemexError("INDEX_INCOMPATIBLE", "Vector segment dimensions do not match the active embedding model.");
    const vectorsStart = HEADER_BYTES;
    const idsStart = vectorsStart + count * dims;
    if (buffer.length < idsStart + count * ID_BYTES)
        throw new MemexError("MODEL_ASSET_CORRUPT", "Vector segment is truncated.");
    const rows = [];
    for (let index = 0; index < count; index += 1) {
        const slice = buffer.subarray(vectorsStart + index * dims, vectorsStart + (index + 1) * dims);
        const vector = new Int8Array(dims);
        for (let byteIndex = 0; byteIndex < dims; byteIndex += 1)
            vector[byteIndex] = slice.readInt8(byteIndex);
        rows.push({ id: buffer.toString("ascii", idsStart + index * ID_BYTES, idsStart + (index + 1) * ID_BYTES), vector });
    }
    return rows;
}
function parseManifest(value) {
    if (!isRecord(value) || value.schemaVersion !== 1 || typeof value.modelId !== "string" || typeof value.modelRevision !== "string" ||
...
function parseManifestChunk(value) {
    if (!isRecord(value) || !isBucket(value.bucket))
        throw new MemexError("MODEL_ASSET_CORRUPT", "Vector store manifest chunk is invalid.");
    return { ref: parseChunkRef(value.ref), bucket: value.bucket };
}
function parseManifestSegment(value) {
    if (!isRecord(value) || !isBucket(value.bucket) || typeof value.file !== "string" || value.file.length === 0 ||
