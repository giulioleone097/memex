...
export function parseWikiState(input) {
    ...
    if (state.schemaVersion !== 1) {
        throw new MemexError("INVALID_STATE", "Unsupported wiki state schema version. Expected 1.");
    }
...
export function parseSourceEnvelope(input) {
    ...
    if (envelope.schemaVersion !== 1) {
        throw new MemexError("INVALID_ARGUMENT", "Unsupported source envelope schema version. Expected 1.");
    }
    ...
    if (envelope.items.length > MAX_ENVELOPE_ITEMS) {
        throw new MemexError("SOURCE_TOO_LARGE", `Source envelope exceeds the ${String(MAX_ENVELOPE_ITEMS)} item limit.`);
    }
    const items = envelope.items.map((item, index) => parseSourceItem(item, index));
    const externalIds = new Set();
    for (const item of items) {
        if (externalIds.has(item.externalId)) {
            throw new MemexError("INVALID_ARGUMENT", "Source envelope contains duplicate externalId values.");
        }
...
function parseOptionalMetadata(item, label) {
    ...
    for (const [key, value] of Object.entries(metadata)) {
        if (!isMetadataValue(value)) {
            throw new MemexError("INVALID_ARGUMENT", `${label} metadata values must be JSON primitives.`);
        }
        if (typeof value === "number" && !Number.isFinite(value)) {
            throw new MemexError("INVALID_ARGUMENT", `${label} metadata numbers must be finite.`);
        }
        if (typeof value === "string" &&
            utf8ByteLength(value) > MAX_METADATA_VALUE_BYTES) {
            throw new MemexError("SOURCE_TOO_LARGE", `${label} metadata value exceeds the ${String(MAX_METADATA_VALUE_BYTES)} byte limit.`);
        }
...
function enforceEnvelopeByteLimit(input, maxBytes, label) {
    ...
    if (utf8ByteLength(serializedValue) > maxBytes) {
        throw new MemexError("SOURCE_TOO_LARGE", `${label} exceeds the ${String(maxBytes)} byte limit.`);
    }
}
...
function requireBoundedString(value, label) {
    ...
    if (utf8ByteLength(parsed) > MAX_ITEM_TEXT_BYTES) {
        throw new MemexError("SOURCE_TOO_LARGE", `${label} exceeds the ${String(MAX_ITEM_TEXT_BYTES)} byte limit.`);
    }
    return parsed;
}
