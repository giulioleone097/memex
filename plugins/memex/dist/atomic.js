...
async function rejectExistingLock(lockPath) {
    try {
        if ((await lstat(lockPath)).isSymbolicLink()) {
            throw new MemexError("SYMLINK_ESCAPE", "Memex lock path must not be a symbolic link.");
        }
    }
    catch (error) {
        if (error instanceof MemexError) {
    ...
    throw new MemexError("LOCKED", "Memex is locked; lock ownership cannot be inferred safely.");
}
async function releaseOwnedLock(lockPath, serializedLock) {
    ...
    try {
        ...
        throw new MemexError("LOCKED", "Memex lock ownership changed before release.");
    }
...
async function assertNotSymlink(filePath) {
    try {
        if ((await lstat(filePath)).isSymbolicLink()) {
            throw new MemexError("SYMLINK_ESCAPE", "Refused to replace a symbolic-link target.");
        }
    }
    catch (error) {
    ...
    while (handle === undefined) {
        try {
            ...
            if (!isAlreadyExists(error))
                throw new MemexError("IO_FAILURE", "Unable to acquire the write lock.");
            if (await recoverStaleLock(lockPath, staleMs))
                continue;
            if (Date.now() >= deadline)
                throw new MemexError("LOCKED", "Store writer is busy.");
            await wait(10);
        }
    }
    try {
...
async function recoverStaleLock(lockPath, staleMs) {
    ...
    try {
        ...
        if (details.isSymbolicLink() || !details.isFile())
            throw new MemexError("SYMLINK_ESCAPE", "Writer lock must be a regular file.");
...
async function releaseLock(lockPath, serialized) {
    try {
        if (await readFile(lockPath, "utf8") !== serialized)
            throw new MemexError("LOCKED", "Writer lock ownership changed before release.");
        await unlink(lockPath);
    }
    catch (error) {
        if (error instanceof MemexError)
...
async function assertRegularDirectory(directory) {
    ...
    if (details.isSymbolicLink() || !details.isDirectory())
        throw new MemexError("SYMLINK_ESCAPE", "Lock directory must not be a symbolic link.");
}
