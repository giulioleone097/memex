...
${END_MARKER}`;
export async function syncInstructionBlocks(workspaceRoot) {
    const changedFiles = [];
    const candidates = await Promise.all(INSTRUCTION_FILES.map(async (file) => {
    ...
    for (const { file, status } of candidates) {
        if (status === "duplicate" || status === "malformed") {
            throw new MemexError("INVALID_STATE", `${file} contains a ${status} Memex instruction block.`);
        }
    }
...
export async function inspectInstructionBlocks(workspaceRoot) {
    return Promise.all(INSTRUCTION_FILES.map(async (file) => ({
        file,
        status: inspectContent(await readOptionalInstructionFile(path.join(workspaceRoot, file))),
    })));
}
...
async function readOptionalInstructionFile(filePath) {
    let handle;
    try {
        handle = await open(filePath, fsConstants.O_RDONLY | fsConstants.O_NOFOLLOW);
        if (!(await handle.stat()).isFile()) {
            throw new MemexError("INVALID_STATE", "Instruction file path must be a regular file.");
        }
        ...
        if (isSymbolicLinkError(error)) {
            throw new MemexError("SYMLINK_ESCAPE", "Instruction file path must not be a symbolic link.");
        }
