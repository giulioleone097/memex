...
export function isSemanticEdgeKind(kind) {
...
export function computeGodNodes(graph, limit) {
    if (!Number.isSafeInteger(limit) || limit < 1) {
        throw new MemexError("INVALID_ARGUMENT", "God node limit must be a positive integer.");
    }
...
export function computeSurprisingConnections(graph, limit) {
    if (!Number.isSafeInteger(limit) || limit < 1) {
        throw new MemexError("INVALID_ARGUMENT", "Surprising-connection limit must be a positive integer.");
    }
