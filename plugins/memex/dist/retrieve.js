...
import { createEvidenceIdentity, UNSCOPED_PROJECT_SCOPE } from "./evidence-identity.js";
const RRF_K = 60;
...
function fuse(lists, identityConfig) {
    ...
    for (const list of lists) {
        list.items.forEach((item, index) => {
            ...
            if (compareRefs(item.ref, existing.ref) < 0)
...
function evidenceIdentityFor(ref, config) {
    const candidate = ref;
...
function resolveIdentityValue(value, ref) {
