...
export async function ingestSource(options) {
    ...
    return withWikiLock(options.location.dataRoot, async () => {
        ...
        if (existingRuns.some((run) => run.envelope.kind !== redacted.kind)) {
            throw new MemexError("INVALID_ARGUMENT", "Source id is already associated with a different source kind.");
        }
...
export async function purgeData(options) {
    ...
    if (scope === "personal-wiki" && options.location.mode !== "personal") {
        throw new MemexError("INVALID_ARGUMENT", "The personal-wiki purge scope requires personal mode.");
    }
...
async function assertPersonalWikiUnlocked(location) {
    ...
    if ((await inspectPath(path.join(location.wikiRoot, ".memex.lock"))) !== null) {
        throw new MemexError("LOCKED", "Memex is locked; purge cannot infer safe ownership.");
    }
}
function canonicalJson(value, ancestors = new WeakSet()) {
    ...
    throw new MemexError("INVALID_ARGUMENT", "Canonical hashing requires a JSON-compatible value.");
}
function withCanonicalGuard(value, ancestors, operation) {
    if (ancestors.has(value)) {
        throw new MemexError("INVALID_ARGUMENT", "Canonical hashing does not accept cyclic values.");
    }
    ancestors.add(value);
    try {
        return operation();
...
function parseRetentionRuns(value) {
    ...
    if (!Number.isInteger(parsed) ||
        ...
        throw new MemexError("INVALID_ARGUMENT", `Source retention must be an integer from 1 to ${String(MAX_SOURCE_RETENTION_RUNS)}.`);
    }
    return parsed;
}
function parsePurgeScope(value) {
    if (!isPurgeScope(value)) {
        throw new MemexError("INVALID_ARGUMENT", "Purge scope must be raw, schedules, personal-wiki, or all.");
    }
    return value;
}
...
function assertRunIdentity(runs, sourceId) {
    ...
    for (const run of runs) {
        if (run.envelope.sourceId !== sourceId ||
            ...
            throw new MemexError("INVALID_STATE", "Persisted source runs contain inconsistent source identity.");
        }
    }
}
async function listSourceDirectories(rawRoot) {
    ...
    if (status.isSymbolicLink()) {
        throw new MemexError("SYMLINK_ESCAPE", "Raw source storage must not be a symbolic link.");
    }
    ...
    try {
        ...
        for (const entry of entries) {
            if (entry.isSymbolicLink()) {
                throw new MemexError("SYMLINK_ESCAPE", "Raw source storage contains a symbolic link.");
            }
...
async function readRunsFromSourceDirectory(sourceDirectory) {
    ...
    if (status.isSymbolicLink()) {
        throw new MemexError("SYMLINK_ESCAPE", "Source run directory must not be a symbolic link.");
    }
    if (!status.isDirectory()) {
    ...
    for (const entry of entries) {
        if (entry.isSymbolicLink()) {
            throw new MemexError("SYMLINK_ESCAPE", "Source runs must not contain symbolic links.");
        }
        ...
        if (canonicalJsonHash(envelope) !== runHash ||
            ...
            throw new MemexError("INVALID_STATE", "Persisted source run integrity check failed.");
        }
...
async function readStoredEnvelope(filePath) {
    ...
    try {
        ...
        try {
            ...
            throw new MemexError("INVALID_STATE", "Persisted source run is not valid JSON.");
        }
        try {
        ...
        if (isSymlinkError(error)) {
            throw new MemexError("SYMLINK_ESCAPE", "Persisted source run must not be a symbolic link.");
        }
...
async function ensureDirectory(directory) {
    ...
    if (existing?.isSymbolicLink()) {
        throw new MemexError("SYMLINK_ESCAPE", "Private Memex storage must not be a symbolic link.");
    }
...
async function unlinkRegularFile(filePath) {
    ...
    if (status.isSymbolicLink()) {
        throw new MemexError("SYMLINK_ESCAPE", "Source retention must not follow symbolic links.");
    }
    if (!status.isFile()) {
        throw new MemexError("INVALID_STATE", "Persisted source run is invalid.");
    }
