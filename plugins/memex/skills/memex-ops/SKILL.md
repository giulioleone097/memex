---
name: memex-ops
description: Diagnose, schedule, recover, purge, uninstall, and explain privacy boundaries for Memex. Use when a user reports unhealthy state, stale locks, schedule problems, path or symlink errors, secret leakage risk, or requests destructive cleanup or plugin removal.
---

# Operate Memex safely

Prefer read-only diagnosis. Require explicit scope and confirmation for destructive operations.

## Host paths

- Codex: derive the plugin root from this SKILL.md path by removing `/skills/memex-ops/SKILL.md`; never derive it from the current working directory.
