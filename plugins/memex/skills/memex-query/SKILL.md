---
name: memex-query
description: Answer questions from grounded Memex page evidence without mutating wiki state. Use when a user asks about architecture, workflows, source locations, operations, domain concepts, integrations, or any fact expected to exist in an initialized Memex.
---
...
6. If freshness materially affects the answer, offer `memex-update` without starting it automatically.
...
- On `NOT_INITIALIZED`, stop and offer `memex-init`.
- On no search results, broaden terms once, then report the evidence gap instead of fabricating an answer.
- On invalid links or state, stop and offer `memex-ops` doctor; do not repair during a query.
