---
name: memex-graph
description: Inspect a repository with the private Memex-native code graph before broad source scans. Use when a user asks about code structure, symbols, dependencies, architecture, impact, changed paths, or repository relationships.
---

# Memex graph

Use the Memex-native graph for compact repository understanding. This workflow is code-mode only; personal mode has no repository graph and must continue through the personal wiki skills.

## Host paths

- Codex: derive the plugin root from this SKILL.md path by removing `/skills/memex-graph/SKILL.md`; never derive it from the current working directory.
...
- For a build, record that the write target is private `~/.memex/data/<workspace-id>/graph/`; source files, wiki files, and instruction files are not mutation targets.
...
- On corrupt or unreadable graph state, preserve the safe error and route recovery to `memex-ops`. Do not delete private graph files manually or treat a partial snapshot as complete.
