...
async function temporaryRoot(name) {
  const root = await mkdtemp(path.join(os.tmpdir(), `memex-concurrency-${name}-`));
  roots.push(root);
  return root;
}
...
test("graph store: a real child-process writer excludes another writer", { timeout: 30_000 }, async (t) => {
  ...
  await writeGraph(resolved.storage, { schemaVersion: 2, workspaceId: "a".repeat(64), generatedAt: "2026-07-11T00:00:00.000Z", source: { dirtyFingerprint: "b".repeat(64), scannerVersion: "memex-graph-v1" }, files: [], nodes: [{ id: repository, kind: "repository", path: ".", name: "repository" }], edges: [], diagnostics: [] }, []);
