...
async function temporaryRoot(label) {
  const root = await mkdtemp(path.join(os.tmpdir(), `memex-enrich-${label}-`));
  roots.push(root);
  return root;
}
...
async function repositoryWithWikiPage() {
  ...
  await mkdir(path.join(root, "memex"));
  await writeFile(path.join(root, "memex", "architecture.md"), "# Architecture\n\nThe worker performs background runs.\n");
  await git(root, ["add", "."]);
...
describe("enrich operation", () => {
  test("requires a prior graph build", async () => {
    ...
    const hash = await pageHash(root, "memex/architecture.md");
  ...
  test("persists a page node grounded by an auto-synthesized source node", async () => {
    ...
    const pageId = createGraphNodeId("page", "memex/architecture.md", "memex/architecture.md");
  ...
  test("references an existing code symbol node by its graph id in a mentions edge", async () => {
    ...
    const result = await enrichGraph({
      ...
      envelope: envelope({
        ...
        edges: [{ kind: "mentions", from: "page:memex/architecture.md:memex/architecture.md", to: runId, confidence: "inferred" }],
      }),
    });
    assert.equal(result.applied, true);
  ...
  test("is a no-op on an unchanged sourceContentHash and rejects an unresolved edge reference", async () => {
    ...
    await assert.rejects(
      enrichGraph({
        root,
        homeDir: home,
        envelope: envelope({ sourceContentHash: hash, edges: [{ kind: "mentions", from: "page:memex/architecture.md:memex/architecture.md", to: "f".repeat(64), confidence: "inferred" }] }),
      }),
      (error) => {
        assert.ok(error instanceof MemexError);
