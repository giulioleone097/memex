...
function createHarness(t, initialState = emptyState(), extraEnv = {}) {
  const directory = mkdtempSync(join(tmpdir(), "memex installer "));
  ...
  for (const entry of initialState.claude.installed) {
    if (entry.id === PLUGIN_ID) {
      entry.version ??= "0.1.0";
      entry.installPath ??= join(claudeConfig, "plugins/cache/memex-local/memex/0.1.0");
    }
  }
    ...
    run(scriptName, args, options = {}) {
      ...
      const result = spawnSync(process.execPath, [join(SCRIPT_DIR, scriptName), ...args], {
        ...
        env: {
          ...
          MEMEX_TEST_LOG: logPath,
          MEMEX_TEST_STATE: statePath,
          MEMEX_TEST_CHILD_PID: childPidPath,
          ...extraEnv,
        },
        shell: false,
...
function createInstalledRuntime(t, client, options = {}) {
  const directory = mkdtempSync(join(tmpdir(), "memex installed runtime "));
  ...
  const expectedRoot = join(
    configRoot,
    "plugins/cache/memex-local/memex",
    version,
  );
  ...
  for (const artifact of artifacts) {
    ...
    if (options.symlink === artifact) {
      ...
      writeFileSync(path, artifact === "bin/memex" ? "#!/bin/sh\n" : "{}\n");
      if (artifact === "bin/memex") chmodSync(path, 0o755);
    }
  }
...
describe("repository-local plugin lifecycle", () => {
  ...
  test("tears down orphaned descendants when a client command fails", async (t) => {
    const harness = createHarness(t, emptyState(), {
      MEMEX_TEST_ORPHAN_ON: "plugin install",
    });
    try {
  ...
  test("caps client output and removes descendants without exposing raw content", async (t) => {
    const harness = createHarness(t, emptyState(), {
      MEMEX_TEST_OVERSIZED_OUTPUT_ON: "plugin install",
    });
    try {
      const result = harness.run("install.mjs", ["--claude", "--json"]);
      assert.equal(result.status, 1, result.stderr);
  ...
  test("times out client commands and removes descendants without exposing raw content", async (t) => {
    const harness = createHarness(t, emptyState(), {
      MEMEX_TEST_TIMEOUT_ON: "plugin marketplace list",
    });
    try {
  ...
  test("resolves standard host config directories when overrides are absent", (t) => {
    const home = mkdtempSync(join(tmpdir(), "memex default home "));
...
describe("repository validator", () => {
  ...
  test("reports missing integration artifacts as structured findings instead of crashing", (t) => {
    const harness = createHarness(t);
    const missingRoot = mkdtempSync(join(tmpdir(), "memex missing integration "));
  ...
  test("detects unsafe paths, placeholders, secrets, modes, attribution, and dist gaps", (t) => {
    const temporaryRoot = mkdtempSync(join(tmpdir(), "memex invalid repository "));
    t.after(() => rmSync(temporaryRoot, { recursive: true, force: true }));

    const pluginRoot = join(temporaryRoot, "plugins/memex");
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        plugins: [{ name: PLUGIN, source: "./plugins/memex" }],
      }),
    );
    writeFileSync(
    ...
    writeFileSync(join(pluginRoot, "bin/memex"), "#!/usr/bin/env node\n");
    chmodSync(join(pluginRoot, "bin/memex"), 0o644);
    writeFileSync(
    ...
    for (const expectedPath of [
      "plugins/memex/README.md",
      "plugins/memex/src/placeholder-tbd.md",
      "plugins/memex/src/placeholder-fixme.md",
      "plugins/memex/src/placeholder-angle.md",
      "plugins/memex/src/placeholder-your.md",
      "plugins/memex/src/placeholder-local.md",
    ]) {
