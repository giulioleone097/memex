...
async function runCli(harness, args, options = {}) {
  ...
  const json = parseJsonDocument(processResult.stdout, `memex ${args[0] ?? ""}`);
...
async function createRepositoryHarness(t, options = {}) {
  const sandboxRoot = await mkdtemp(join(tmpdir(), "memex real e2e "));
  ...
  if (options.gitNexusTripwire) {
    ...
    await writeFile(
      ...
      "#!/usr/bin/env node\nrequire('node:fs').appendFileSync(process.env.MEMEX_GITNEXUS_TRIPWIRE, `${JSON.stringify(process.argv.slice(2))}\\n`);\nprocess.exit(97);\n",
    );
    await chmod(join(tripwireBin, "gitnexus"), 0o755);
    harness.tripwireLog = tripwireLog;
    harness.env = {
      ...env,
      MEMEX_GITNEXUS_TRIPWIRE: tripwireLog,
  ...
  await git(harness, ["config", "user.name", "Memex E2E"]);
  await git(harness, ["config", "user.email", "memex-e2e@example.test"]);
...
async function wikiDigest(repositoryRoot) {
  const wikiRoot = join(repositoryRoot, "memex");
...
describe("Memex real runtime journey", () => {
  test("builds, updates, queries, schedules, and purges a grounded repository wiki across processes", async (t) => {
    ...
    const wikiRoot = join(harness.repositoryRoot, "memex");
...
`,
  ...
  test("slice 2c: report/communities/path/explain are deterministic across two runs and mirrored over MCP", async (t) => {
    ...
    const reportPageRaw = await readFile(join(harness.repositoryRoot, "memex", "graph-report.md"), "utf8");
  ...
  test("enrich grounds a wiki page in the graph with a real code-symbol mention, verified by check", async (t) => {
    ...
    const pagePath = join(harness.repositoryRoot, "memex", "architecture.md");
    ...
    const enrichEnvelope = {
      ...
      sourcePath: "memex/architecture.md",
      sourceContentHash: pageHash,
      nodes: [{ kind: "page", name: "memex/architecture.md", path: "memex/architecture.md" }],
      edges: [{ kind: "mentions", from: "page:memex/architecture.md:memex/architecture.md", to: symbolNode.id, confidence: "inferred" }],
    };
...
describe("Memex process-boundary security regressions", () => {
  ...
  test("rejects a wiki symlink escape without modifying its target", async (t) => {
    ...
    await symlink(outsidePage, join(harness.repositoryRoot, "memex", "escape.md"));
  ...
  test("stores prompt-injection-shaped source as inert redacted data", async (t) => {
    ...
    const beforeState = await readFile(
      join(harness.repositoryRoot, "memex", ".last-update.json"),
      "utf8",
    );
    ...
    const persisted = await directoryText(join(harness.homeRoot, ".memex", "data"));
    assert.match(persisted, /IGNORE ALL PREVIOUS INSTRUCTIONS/u);
    assert.match(persisted, /redact/iu);
    assert.doesNotMatch(persisted, /sk-test-memex-secret-1234567890/u);
    assert.doesNotMatch(result.stdout, /sk-test-memex-secret-1234567890|IGNORE ALL PREVIOUS/u);
    assert.equal(await wikiDigest(harness.repositoryRoot), beforeDigest);
    assert.equal(
      await readFile(join(harness.repositoryRoot, "memex", ".last-update.json"), "utf8"),
      beforeState,
    );
  });
  ...
  test("rejects an oversized source envelope before private persistence", async (t) => {
    ...
    assert.equal((await listFiles(join(harness.homeRoot, ".memex", "data"))).length, 0);
  });
});
