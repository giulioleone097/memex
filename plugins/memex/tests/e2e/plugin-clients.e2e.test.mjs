...
const PLUGIN_ID = "memex@memex-local";
...
function createLifecycleHarness(t) {
  const root = mkdtempSync(join(tmpdir(), "memex client lifecycle "));
...
function createIsolatedLiveEnvironment(t, client) {
  const root = mkdtempSync(join(tmpdir(), `memex live ${client} `));
  const home = join(root, "home");
  ...
  delete env.MEMEX_TEST_LOG;
  delete env.MEMEX_TEST_STATE;
  t.after(() => rmSync(root, { recursive: true, force: true }));
  return { root, env };
}
...
function assertInstalledArtifacts(client, root) {
  ...
  if (process.platform !== "win32") {
    assert.notEqual(statSync(join(root, "bin/memex")).mode & 0o111, 0);
  }
}
...
const expectedUninstallArgv = [
  ...
  {
    client: "codex",
    argv: ["plugin", "marketplace", "remove", "memex-local", "--json"],
  },
  { client: "claude", argv: ["plugin", "list", "--json"] },
  {
    client: "claude",
  ...
  {
    client: "claude",
    argv: ["plugin", "marketplace", "remove", "memex-local", "--scope", "user"],
  },
];

describe("Memex plugin clients", { concurrency: 1 }, () => {
  ...
  test(
    "installs, lists, and removes the plugin with the real Codex client in an isolated home",
    {
      skip: LIVE_SMOKE_ENABLED
        ? false
        : "MEMEX_RUN_CLIENT_SMOKE is not 1; live Codex smoke intentionally not executed.",
    },
    (t) => {
      ...
      assert.match(list.stdout, /memex/u);
  ...
  test(
    "validates, installs, lists, and removes the plugin with the real Claude client in an isolated home",
    {
      skip: LIVE_SMOKE_ENABLED
        ? false
        : "MEMEX_RUN_CLIENT_SMOKE is not 1; live Claude smoke intentionally not executed.",
    },
    (t) => {
      ...
      assert.match(details.stdout, /memex/u);
  ...
  test(
    "runs both installed caches after their disposable marketplace source is removed",
    {
      skip: LIVE_SMOKE_ENABLED
        ? false
        : "MEMEX_RUN_CLIENT_SMOKE is not 1; live installed-cache smoke intentionally not executed.",
    },
    (t) => {
      ...
      for (const validationRoot of [source, join(source, "plugins/memex")]) {
      ...
      const install = run(
        process.execPath,
        [join(source, "plugins/memex/scripts/install.mjs"), "--all", "--json"],
