...
async function temporaryRoot(label) {
  const root = await mkdtemp(path.join(os.tmpdir(), `memex-analysis-store-${label}-`));
  roots.push(root);
  return root;
}
...
describe("analysis-store: communities snapshot persistence", () => {
  ...
  test("analysis-store: rejects a malformed snapshot on read", () => {
    assert.throws(() => parseCommunitiesSnapshot({ schemaVersion: 2 }), MemexError);
    assert.throws(() => parseCommunitiesSnapshot({ schemaVersion: 1, generation: "g", generatedAt: "t", communities: [{}], membership: {} }), MemexError);
  });
  ...
  test("analysis-store: throws NOT_INITIALIZED reading a missing snapshot", async () => {
    ...
    const home = await temporaryRoot("home");
    const resolved = await resolveAnalysisStorage(root, home);
    await assert.rejects(readCommunitiesSnapshot(resolved.storage), (error) => error instanceof MemexError && error.code === "NOT_INITIALIZED");
  });
});
