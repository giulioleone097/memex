...
describe("storage", () => {
  test("storage: resolves deterministic code and personal wiki locations", async () => {
    ...
    assert.equal(code.wikiRoot, path.join(code.workspaceRoot, "memex"));
    ...
    assert.equal(personal.wikiRoot, path.join(canonicalHome, ".memex", "wiki"));
    assert.equal(
      personal.statePath,
      path.join(canonicalHome, ".memex", ".last-update.json"),
    );
    assert.equal(
      personal.dataRoot,
      path.join(canonicalHome, ".memex", "data", "personal"),
    );
  });
  ...
  test("storage: rejects a private data symlink that escapes the configured home", async () => {
    ...
    const privateRoot = path.join(home, ".memex");
  ...
  test("storage: initializes required pages idempotently without overwriting user content", async () => {
    ...
    const wikiRoot = path.join(repository, "memex");
  ...
  test("storage: rejects a directory masquerading as a required wiki page", async () => {
    ...
    await mkdir(path.join(repository, "memex", "quickstart.md"), {
      recursive: true,
    });
  ...
  test("storage: rejects missing and malformed state with typed errors", async () => {
    ...
    await expectMemexError(readState(location), "NOT_INITIALIZED");
  ...
  test("storage: serializes concurrent processes and never guesses stale lock ownership", async () => {
    ...
    await expectMemexError(withWikiLock(root, async () => undefined), "LOCKED");
    const [exitCode] = await once(child, "exit");
    assert.equal(exitCode, 0);
    assert.equal(await withWikiLock(root, async () => "released"), "released");

    const staleLock = path.join(root, ".memex.lock");
