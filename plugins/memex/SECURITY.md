...
Security fixes target the current `0.2.x` plugin release line. Upgrade both host installations before reporting behavior that may already be fixed.
...
- Code-mode writes are confined to the selected repository wiki, Memex-owned instruction blocks, and private runtime data. Personal-mode writes are confined to `~/.memex/`.
...
The graph is Memex-owned and has no external graph dependency, provider, service, binary, compatible data format, or first-use download. Before graph reads or writes, repository and private-data paths are canonicalized; repository symlinks are not followed into the scan, and symlink escapes fail closed. Dependency, generated, VCS, wiki, and private Memex paths are excluded by deterministic rules.
...
Graph build reads source for parsing but never executes repository code and never stores source bodies. It writes only private atomic index data under `~/.memex/data/<workspace-id>/graph/`; source, wiki, instruction, dependency, and credential files remain outside its mutation boundary.
