...
export async function reindexCodeSymbols(root, index, homeDir, ports = {}) {
    ...
    const refs = chunkSymbols(symbols).map((ref) => {
        ...
        if (node === undefined)
            throw new OpenWikiError("INVALID_STATE", "Chunked symbol node disappeared during reindexing.");
...
async function reindexChunks(dataRoot, refs, ports) {
    ...
    const entries = changed.map(({ ref }, index) => {
        ...
        if (vector === undefined)
            throw new OpenWikiError("EMBEDDING_FAILURE", "Embedder returned fewer vectors than requested.");
        return { ref, vector };
    });
