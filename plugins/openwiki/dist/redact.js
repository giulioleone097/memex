...
const SENSITIVE_KEY_PATTERN = /(?:^|[_-])(?:authorization|api[_-]?key|access[_-]?token|refresh[_-]?token|auth[_-]?token|client[_-]?secret|password|passwd|private[_-]?key|secret|token)(?:$|[_-])/iu;
...
const NAMED_SECRET_PATTERN = /\b(?:api[_-]?key|access[_-]?token|refresh[_-]?token|auth[_-]?token|client[_-]?secret|password|passwd|private[_-]?key|secret|token)\s*[:=]\s*["']?[A-Za-z0-9._~+/=-]{8,}["']?/giu;
...
function redactValue(value, ancestors) {
    ...
    if (Array.isArray(value)) {
        return withObjectGuard(value, ancestors, () => value.map((item) => redactValue(item, ancestors)));
    }
    if (isPlainRecord(value)) {
        return withObjectGuard(value, ancestors, () => redactRecord(value, ancestors));
    }
    throw new OpenWikiError("INVALID_ARGUMENT", "Sensitive data redaction requires a JSON-compatible value.");
}
function redactRecord(record, ancestors) {
    const redacted = {};
    for (const [rawKey, rawValue] of Object.entries(record)) {
        const key = rawKey.replaceAll("\0", "");
        if (Object.hasOwn(redacted, key)) {
            throw new OpenWikiError("INVALID_ARGUMENT", "Sensitive data redaction produced duplicate object keys.");
        }
...
function inspectValue(value, ancestors) {
    if (typeof value === "string") {
    ...
    if (Array.isArray(value)) {
        return withObjectGuard(value, ancestors, () => value.some((item) => inspectValue(item, ancestors)));
    }
    if (isPlainRecord(value)) {
...
function withObjectGuard(value, ancestors, operation) {
    if (ancestors.has(value)) {
        throw new OpenWikiError("INVALID_ARGUMENT", "Sensitive data redaction does not accept cyclic values.");
    }
    ancestors.add(value);
    try {
        return operation();
