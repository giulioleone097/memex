...
function parseSchedule(input, code) {
    ...
    if (containsSensitive(parsed)) {
        throw new OpenWikiError(code, "Schedule intent must not contain credential-shaped values.");
    }
    return parsed;
}
function parseScheduleStore(input) {
    ...
    for (const schedule of schedules) {
        if (ids.has(schedule.id)) {
            throw new OpenWikiError("INVALID_STATE", "Schedule state contains duplicate ids.");
        }
        ids.add(schedule.id);
    }
    schedules.sort((left, right) => compareStrings(left.id, right.id));
...
async function readScheduleStore(location) {
    ...
    try {
        ...
        if (isSymlinkError(error)) {
            throw new OpenWikiError("SYMLINK_ESCAPE", "Schedule state must not be a symbolic link.");
        }
...
async function ensureDataRoot(dataRoot) {
    ...
    if (existing?.isSymbolicLink()) {
        throw new OpenWikiError("SYMLINK_ESCAPE", "Schedule storage must not be a symbolic link.");
    }
