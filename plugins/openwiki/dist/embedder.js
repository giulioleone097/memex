...
async function verifyVendorEntry(vendorRoot, entry) {
    if (entry.parts === undefined) {
        if (entry.sha256 === undefined)
            throw new OpenWikiError("MODEL_ASSET_CORRUPT", `Vendor asset manifest entry for ${entry.path} declares neither sha256 nor parts.`);
...
async function verifyVendorFile(vendorRoot, relativePath, expectedSha256, expectedBytes) {
    ...
    if (size !== expectedBytes)
        throw new OpenWikiError("MODEL_ASSET_CORRUPT", `Vendor asset size mismatch: ${relativePath}.`);
    const digest = await sha256File(absolute);
    if (digest !== expectedSha256.toLowerCase())
        throw new OpenWikiError("MODEL_ASSET_CORRUPT", `Vendor asset checksum mismatch: ${relativePath}.`);
}
...
export async function loadModelBuffer(vendorRoot, manifest, logicalPath) {
    ...
    if (entry === undefined)
        throw new OpenWikiError("MODEL_ASSET_MISSING", `Vendor manifest has no entry for ${logicalPath}.`);
    if (entry.parts === undefined) {
        if (entry.sha256 === undefined)
            throw new OpenWikiError("MODEL_ASSET_MISSING", `Vendor manifest entry for ${logicalPath} declares neither a single file nor parts.`);
    ...
    if (entry.assembledSha256 === undefined)
        throw new OpenWikiError("MODEL_ASSET_CORRUPT", `Vendor manifest is missing the assembled checksum for ${logicalPath}.`);
    const actual = createHash("sha256").update(assembled).digest("hex");
    if (actual !== entry.assembledSha256.toLowerCase())
        throw new OpenWikiError("MODEL_ASSET_CORRUPT", `Assembled checksum mismatch for ${logicalPath}.`);
    return assembled;
}
    ...
    return {
        async createSession(modelPathOrBuffer) {
            ...
            if (typeof record.run !== "function")
                throw new OpenWikiError("MODEL_ASSET_CORRUPT", "ONNX Runtime session is missing run().");
...
async function runInference(runtime, session, tokenizer, text) {
    ...
    if (ids.length === 0)
        throw new OpenWikiError("EMBEDDING_FAILURE", "Tokenizer produced an empty sequence.");
    ...
    if (hiddenTensor === undefined)
        throw new OpenWikiError("EMBEDDING_FAILURE", "ONNX Runtime returned no output tensor.");
    const [batch, tensorSeqLen, dims] = hiddenTensor.dims;
    if (batch !== 1 || tensorSeqLen !== seqLen || dims !== DIMS)
        throw new OpenWikiError("EMBEDDING_FAILURE", "ONNX Runtime output tensor has an unexpected shape.");
...
function parseManifest(value) {
    if (!isRecord(value) || !Array.isArray(value.assets))
        throw new OpenWikiError("MODEL_ASSET_CORRUPT", "Vendor asset manifest is invalid.");
...
function parseManifestEntry(value) {
    ...
    if (hasSingleHash) {
        ...
        if (typeof sha256 !== "string" || !/^[a-f0-9]{64}$/iu.test(sha256))
            throw new OpenWikiError("MODEL_ASSET_CORRUPT", `Vendor asset manifest entry for ${value.path} has an invalid sha256.`);
        return { ...base, sha256 };
    }
    const assembledSha256 = value.assembled_sha256;
    if (typeof assembledSha256 !== "string" || !/^[a-f0-9]{64}$/iu.test(assembledSha256))
        throw new OpenWikiError("MODEL_ASSET_CORRUPT", `Vendor asset manifest entry for ${value.path} has an invalid assembled_sha256.`);
    ...
    parts.forEach((part, index) => {
        if (!part.path.endsWith(`.part${String(index)}`))
            throw new OpenWikiError("MODEL_ASSET_CORRUPT", `Vendor asset manifest parts for ${entryPath} must be declared in order part0, part1, ....`);
    });
...
function asRecord(value) {
    if (!isRecord(value))
        throw new OpenWikiError("MODEL_ASSET_CORRUPT", "Vendored ONNX Runtime Web module returned an invalid value.");
    return value;
}
function asTensor(value) {
    const record = asRecord(value);
    if (!Array.isArray(record.dims) || record.data === undefined || record.data === null)
        throw new OpenWikiError("MODEL_ASSET_CORRUPT", "ONNX Runtime Tensor has an unexpected shape.");
