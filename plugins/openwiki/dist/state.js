...
import { parseWikiState } from "./contracts.js";
import { OpenWikiError } from "./errors.js";
const MAX_STATE_BYTES = 1024 * 1024;
export async function readState(location) {
    let handle;
    try {
        handle = await open(location.statePath, fsConstants.O_RDONLY | fsConstants.O_NOFOLLOW);
        const fileStat = await handle.stat();
        if (!fileStat.isFile() || fileStat.size > MAX_STATE_BYTES) {
            throw new OpenWikiError("INVALID_STATE", "Wiki state file is invalid.");
        }
        ...
        try {
            ...
            throw new OpenWikiError("INVALID_STATE", "Wiki state is not valid JSON.");
        }
        ...
        if (state.mode !== location.mode ||
            ...
            throw new OpenWikiError("INVALID_STATE", "Wiki state does not match the resolved wiki location.");
        }
        return state;
    }
    catch (error) {
        ...
        if (isSymbolicLinkError(error)) {
            throw new OpenWikiError("SYMLINK_ESCAPE", "Wiki state path must not be a symbolic link.");
        }
        throw new OpenWikiError("IO_FAILURE", "Unable to read wiki state.");
    }
    finally {
...
export async function writeState(location, state) {
    ...
    await atomicWriteFile(location.statePath, `${JSON.stringify(validated, null, 2)}\n`);
}
export async function tryReadState(location) {
    try {
        ...
        if (error instanceof OpenWikiError && error.code === "NOT_INITIALIZED") {
            return null;
        }
        throw error;
    }
