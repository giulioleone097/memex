    ...
    return withWikiLock(location.wikiRoot, async () => {
        ...
        for (const page of REQUIRED_WIKI_PAGES) {
            ...
            try {
                ...
                throw new OpenWikiError("IO_FAILURE", "Required OpenWiki template could not be read.");
            }
...
async function walkMarkdown(root, directory, pages) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isSymbolicLink()) {
            throw new OpenWikiError("SYMLINK_ESCAPE", "Wiki content must not contain symbolic links.");
        }
...
async function readFileNoFollow(filePath, missingCode, missingMessage) {
    ...
    try {
        ...
        if (isSymbolicLinkError(error)) {
            throw new OpenWikiError("SYMLINK_ESCAPE", "Wiki page path must not be a symbolic link.");
        }
...
async function pathExists(filePath) {
    ...
    try {
        handle = await open(filePath, fsConstants.O_RDONLY | fsConstants.O_NOFOLLOW);
        if (!(await handle.stat()).isFile()) {
            throw new OpenWikiError("IO_FAILURE", "Required wiki page path is not a regular file.");
        }
        return true;
    }
    catch (error) {
