...
async function checkLocks(location) {
    try {
        const lockPaths = [
            path.join(location.wikiRoot, ".openwiki.lock"),
            path.join(location.dataRoot, ".openwiki.lock"),
        ];
        for (const lockPath of lockPaths) {
            if ((await lstatOrNull(lockPath)) !== null) {
                return fail("locks", "An OpenWiki operation lock is active.");
            }
        }
        return pass("locks", "No OpenWiki operation lock is active.");
    }
    catch {
        return fail("locks", "OpenWiki lock state could not be inspected safely.");
    }
}
...
async function collectFilesNoFollow(target, files) {
    ...
    if (status.isSymbolicLink()) {
        throw new OpenWikiError("SYMLINK_ESCAPE", "Doctor secret scan must not follow symbolic links.");
    }
    if (status.isFile()) {
        files.push(target);
        return;
    ...
    if (!status.isDirectory()) {
        throw new OpenWikiError("INVALID_STATE", "Private data contains an invalid entry.");
    }
...
async function readJsonFileNoFollow(filePath, byteLimit) {
    ...
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
        throw new OpenWikiError("INVALID_STATE", "JSON configuration is invalid.");
    }
    return parsed;
}
async function readTextFileNoFollow(filePath, byteLimit) {
    ...
    try {
        ...
        if (!fileStat.isFile() || fileStat.size > byteLimit) {
            throw new OpenWikiError("INVALID_STATE", "Inspected file is invalid.");
        }
