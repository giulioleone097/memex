...
import { OPENWIKI_OPERATIONS, dispatch } from "./adapter.js";
...
function tool(name, description, inputSchema, annotationValues) {
    if (annotationValues.length !== 4)
        throw new Error("OpenWiki MCP tool annotations must be complete.");
    return {
        name,
        description,
        inputSchema,
...
stdin.on("end", () => {
    if (buffer.length > 0)
        queue = queue.then(() => handle(buffer.replace(/\r$/u, "")));
});
