...
export async function readRepositoryFile(root, file) {
    if (!safeRelativePath(file.path))
        throw new OpenWikiError("INVALID_ARGUMENT", "Graph repository path is invalid.");
    ...
    if (body.includes(0))
        throw new OpenWikiError("UNSUPPORTED_SOURCE", "Graph scanner refuses binary repository files.");
...
export async function readManifest(storage) {
    ...
    if (first === undefined)
        throw new OpenWikiError("NOT_INITIALIZED", "No recoverable OpenWiki graph manifest exists.");
    ...
    while (handle === undefined) {
        try {
            ...
            if (!isAlreadyExists(error))
                throw new OpenWikiError("IO_FAILURE", "Unable to acquire the graph writer lock.");
            if (await recoverStaleGraphLock(storage.writeLockPath))
                continue;
            if (Date.now() >= deadline)
                throw new OpenWikiError("LOCKED", "OpenWiki graph writer is busy.");
            await wait(10);
        }
    }
    try {
...
async function writeGraphUnlocked(storage, graph, shards, enrichmentShards) {
    ...
    if (!published) {
        ...
        if (existing.generation !== generation)
            throw new OpenWikiError("INVALID_STATE", "Graph generation publication changed during write.");
...
function parseManifest(value) {
    ...
    if (JSON.stringify(shards) !== JSON.stringify(value.shards))
        throw new OpenWikiError("INVALID_STATE", "Graph manifest shards are not canonical.");
    ...
    if (JSON.stringify(enrichmentShards) !== JSON.stringify(enrichmentShardsInput))
        throw new OpenWikiError("INVALID_STATE", "Graph manifest enrichment shards are not canonical.");
...
function parseManifestEnrichmentShard(value) {
    if (!isRecord(value) || !safeRelativePath(value.sourcePath) || typeof value.sourceContentHash !== "string" || !/^[a-f0-9]{64}$/iu.test(value.sourceContentHash) || !safeStoredName(value.shard))
        throw new OpenWikiError("INVALID_STATE", "Graph manifest enrichment shard is invalid.");
...
function parseManifestIndex(value, generation) {
    ...
    const buckets = (entry) => {
        if (!Array.isArray(entry))
            throw new OpenWikiError("INVALID_STATE", "Graph manifest index buckets are invalid.");
        const names = [];
        ...
        if (new Set(ordered).size !== ordered.length || JSON.stringify(entry) !== JSON.stringify(ordered))
            throw new OpenWikiError("INVALID_STATE", "Graph manifest index buckets are not canonical.");
        return ordered;
    };
...
function parseManifestShard(value) {
    if (!isRecord(value) || !safeRelativePath(value.path) || typeof value.contentHash !== "string" || !/^[a-f0-9]{64}$/iu.test(value.contentHash) || typeof value.sourceId !== "string" || value.sourceId.length === 0 || !safeStoredName(value.shard))
        throw new OpenWikiError("INVALID_STATE", "Graph manifest shard is invalid.");
...
async function recoverStaleGraphLock(lockPath) {
    let serialized;
    try {
        const details = await lstat(lockPath);
        if (details.isSymbolicLink() || !details.isFile())
            throw new OpenWikiError("SYMLINK_ESCAPE", "Graph writer lock must be a regular file.");
    ...
    if (lock === undefined || Date.now() - lock.createdAt < GRAPH_STALE_LOCK_MS || processAlive(lock.pid))
        return false;
    try {
...
async function releaseGraphLock(lockPath, serialized) {
    try {
        if (await readFile(lockPath, "utf8") !== serialized)
            throw new OpenWikiError("LOCKED", "Graph writer lock ownership changed before release.");
        await unlink(lockPath);
    }
    catch (error) {
        if (error instanceof OpenWikiError)
...
function parseLock(serialized) {
    try {
        ...
        if (!isRecord(value) || value.schemaVersion !== 1 || typeof value.pid !== "number" || !Number.isSafeInteger(value.pid) || value.pid < 1 || typeof value.createdAt !== "string" || typeof value.token !== "string")
            return undefined;
...
function manifestGenerationPath(storage, generation) {
    if (!safeGeneration(generation))
        throw new OpenWikiError("INVALID_STATE", "Graph generation is invalid.");
...
function manifestSnapshotPath(storage, manifest) {
    if (manifest.snapshot !== `${manifest.generation}/snapshot.json`)
        throw new OpenWikiError("INVALID_STATE", "Graph snapshot path is invalid.");
    return path.join(storage.generationRoot, manifest.snapshot);
}
function confinedStoredName(root, name) {
    if (!safeStoredName(name))
        throw new OpenWikiError("INVALID_STATE", "Graph storage entry is invalid.");
...
function parseShard(value) {
    if (!isRecord(value) || !safeRelativePath(value.path) || typeof value.language !== "string" || typeof value.contentHash !== "string" || !/^[a-f0-9]{64}$/iu.test(value.contentHash) || typeof value.sourceId !== "string" || value.sourceId.length === 0 || typeof value.size !== "number" || !Number.isSafeInteger(value.size) || value.size < 0 || !isRecord(value.scan))
        throw new OpenWikiError("INVALID_STATE", "Graph shard is invalid.");
    ...
    const strings = (key) => { const candidate = scan[key]; if (!Array.isArray(candidate) || !candidate.every((entry) => typeof entry === "string"))
        throw new OpenWikiError("INVALID_STATE", "Graph shard is invalid."); return candidate; };
...
function parseShardSymbol(value) {
    if (!isRecord(value) || typeof value.name !== "string" || typeof value.qualifiedName !== "string" || typeof value.scope !== "string" || typeof value.kind !== "string" || !positiveLine(value.startLine) || !positiveLine(value.endLine) || value.startLine > value.endLine || typeof value.exported !== "boolean")
        throw new OpenWikiError("INVALID_STATE", "Graph shard symbol is invalid.");
...
function parseShardRelation(value) {
    if (!isRecord(value) || !isRelationKind(value.kind) || typeof value.fromQualifiedName !== "string" || typeof value.target !== "string" || !positiveLine(value.line) || (value.confidence !== "resolved" && value.confidence !== "heuristic"))
        throw new OpenWikiError("INVALID_STATE", "Graph shard relation is invalid.");
...
function parseShardDiagnostic(value) {
    if (!isRecord(value) || !safeRelativePath(value.path) || typeof value.code !== "string" || typeof value.message !== "string")
        throw new OpenWikiError("INVALID_STATE", "Graph shard diagnostic is invalid.");
...
function isAlreadyExists(error) { return error instanceof Error && "code" in error && error.code === "EEXIST"; }
...
async function assertRegularDirectory(directory) { const details = await lstat(directory); if (details.isSymbolicLink() || !details.isDirectory())
    throw new OpenWikiError("SYMLINK_ESCAPE", "Graph storage directory must not be a symbolic link."); }
async function isRegularFile(file) { try {
    const details = await lstat(file);
    if (details.isSymbolicLink())
        throw new OpenWikiError("SYMLINK_ESCAPE", "Graph manifest must not be a symbolic link.");
    ...
    if (isNotFound(error))
        return false;
    throw new OpenWikiError("IO_FAILURE", "Unable to probe graph storage.");
} }
