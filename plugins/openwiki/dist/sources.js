...
import { MAX_ENVELOPE_BYTES, parseSourceEnvelope, } from "./contracts.js";
...
export const PURGE_SCOPES = ["raw", "schedules", "personal-wiki", "all"];
...
export async function ingestSource(options) {
    ...
    return withWikiLock(options.location.dataRoot, async () => {
        ...
        if (existingRuns.some((run) => run.envelope.kind !== redacted.kind)) {
            throw new OpenWikiError("INVALID_ARGUMENT", "Source id is already associated with a different source kind.");
        }
        ...
        const novelItems = redacted.items.filter((item) => !knownItems.has(itemIdentity(redacted.sourceId, item)));
        ...
        if (novelItems.length > 0) {
            await atomicWriteFile(path.join(sourceDirectory, `${runHash}.json`), `${JSON.stringify(persistedEnvelope)}\n`);
            stored = true;
        }
...
export async function listSources(location) {
...
export async function purgeData(options) {
    const scope = parsePurgeScope(options.scope);
    if (scope === "personal-wiki" && options.location.mode !== "personal") {
        throw new OpenWikiError("INVALID_ARGUMENT", "The personal-wiki purge scope requires personal mode.");
    }
    if ((scope === "personal-wiki" || scope === "all") &&
...
async function assertPersonalWikiUnlocked(location) {
    const wikiStatus = await inspectPath(location.wikiRoot);
    if (wikiStatus === null || wikiStatus.isSymbolicLink() || !wikiStatus.isDirectory()) {
        return;
    }
    if ((await inspectPath(path.join(location.wikiRoot, ".openwiki.lock"))) !== null) {
        throw new OpenWikiError("LOCKED", "OpenWiki is locked; purge cannot infer safe ownership.");
    }
}
function canonicalJson(value, ancestors = new WeakSet()) {
    ...
    if (Array.isArray(value)) {
        return withCanonicalGuard(value, ancestors, () => `[${value.map((item) => canonicalJson(item, ancestors)).join(",")}]`);
    }
    ...
    throw new OpenWikiError("INVALID_ARGUMENT", "Canonical hashing requires a JSON-compatible value.");
}
function withCanonicalGuard(value, ancestors, operation) {
    if (ancestors.has(value)) {
        throw new OpenWikiError("INVALID_ARGUMENT", "Canonical hashing does not accept cyclic values.");
    }
    ancestors.add(value);
    try {
        return operation();
...
function itemIdentity(sourceId, item) {
    return `${sourceId}\0${item.externalId}\0${canonicalJsonHash(item)}`;
}
function parseRetentionRuns(value) {
    const parsed = value ?? DEFAULT_SOURCE_RETENTION_RUNS;
    if (!Number.isInteger(parsed) ||
        parsed < 1 ||
        parsed > MAX_SOURCE_RETENTION_RUNS) {
        throw new OpenWikiError("INVALID_ARGUMENT", `Source retention must be an integer from 1 to ${String(MAX_SOURCE_RETENTION_RUNS)}.`);
    }
    return parsed;
}
function parsePurgeScope(value) {
    if (!isPurgeScope(value)) {
        throw new OpenWikiError("INVALID_ARGUMENT", "Purge scope must be raw, schedules, personal-wiki, or all.");
    }
    return value;
}
function getPrivatePurgeScopes(scope) {
...
async function retainLatestRuns(runs, retentionRuns) {
...
function assertRunIdentity(runs, sourceId) {
    const sourceDirectoryName = canonicalJsonHash(sourceId);
    const persistedKind = runs[0]?.envelope.kind;
    for (const run of runs) {
        if (run.envelope.sourceId !== sourceId ||
            run.envelope.kind !== persistedKind ||
            path.basename(path.dirname(run.filePath)) !== sourceDirectoryName) {
            throw new OpenWikiError("INVALID_STATE", "Persisted source runs contain inconsistent source identity.");
        }
    }
}
async function listSourceDirectories(rawRoot) {
    const status = await inspectPath(rawRoot);
    if (status === null) {
        return [];
    }
    if (status.isSymbolicLink()) {
        throw new OpenWikiError("SYMLINK_ESCAPE", "Raw source storage must not be a symbolic link.");
    }
    if (!status.isDirectory()) {
        throw new OpenWikiError("INVALID_STATE", "Raw source storage is invalid.");
    }
    try {
        ...
        for (const entry of entries) {
            if (entry.isSymbolicLink()) {
                throw new OpenWikiError("SYMLINK_ESCAPE", "Raw source storage contains a symbolic link.");
            }
...
async function readRunsFromSourceDirectory(sourceDirectory) {
    const status = await inspectPath(sourceDirectory);
    if (status === null) {
        return [];
    }
    if (status.isSymbolicLink()) {
        throw new OpenWikiError("SYMLINK_ESCAPE", "Source run directory must not be a symbolic link.");
    }
    if (!status.isDirectory()) {
        throw new OpenWikiError("INVALID_STATE", "Source run directory is invalid.");
    }
    let entries;
    try {
    ...
    for (const entry of entries) {
        if (entry.isSymbolicLink()) {
            throw new OpenWikiError("SYMLINK_ESCAPE", "Source runs must not contain symbolic links.");
        }
        const runHash = entry.name.endsWith(".json") ? entry.name.slice(0, -5) : "";
        if (!entry.isFile() || !HASH_PATTERN.test(runHash)) {
            throw new OpenWikiError("INVALID_STATE", "Persisted source run is invalid.");
        }
        const filePath = path.join(sourceDirectory, entry.name);
        const envelope = await readStoredEnvelope(filePath);
        if (canonicalJsonHash(envelope) !== runHash ||
            canonicalJsonHash(envelope.sourceId) !== path.basename(sourceDirectory)) {
            throw new OpenWikiError("INVALID_STATE", "Persisted source run integrity check failed.");
        }
        runs.push({ filePath, runHash, envelope });
    }
    return runs;
}
async function readStoredEnvelope(filePath) {
    let handle;
    try {
        ...
        try {
            ...
            throw new OpenWikiError("INVALID_STATE", "Persisted source run is not valid JSON.");
        }
        try {
        ...
        if (isSymlinkError(error)) {
            throw new OpenWikiError("SYMLINK_ESCAPE", "Persisted source run must not be a symbolic link.");
        }
...
async function ensureDirectory(directory) {
    ...
    if (existing?.isSymbolicLink()) {
        throw new OpenWikiError("SYMLINK_ESCAPE", "Private OpenWiki storage must not be a symbolic link.");
    }
    if (existing !== null && !existing.isDirectory()) {
        throw new OpenWikiError("IO_FAILURE", "Private OpenWiki storage is invalid.");
    }
    if (existing !== null) {
        return;
    }
    try {
        ...
        throw mapStorageError(error, "Unable to create private OpenWiki storage.");
    }
}
async function directoryExistsWithoutSymlink(directory) {
...
async function inspectPath(filePath) {
    try {
        ...
        throw mapStorageError(error, "Unable to inspect private OpenWiki storage.");
    }
}
async function unlinkRegularFile(filePath) {
    ...
    if (status.isSymbolicLink()) {
        throw new OpenWikiError("SYMLINK_ESCAPE", "Source retention must not follow symbolic links.");
    }
    if (!status.isFile()) {
        throw new OpenWikiError("INVALID_STATE", "Persisted source run is invalid.");
    }
...
async function removeTreeNoFollow(target) {
    ...
    try {
        ...
        throw mapStorageError(error, "Unable to purge OpenWiki data.");
    }
}
function mapStorageError(error, message) {
    if (error instanceof OpenWikiError) {
        return error;
    }
    return new OpenWikiError("IO_FAILURE", message);
}
