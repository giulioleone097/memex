...
export async function loadTokenizer(tokenizerJsonPath) {
    let raw;
    try {
    ...
    return { encode: (text) => encodeWithConfig(config, text) };
}
function parseTokenizerConfig(value) {
    ...
    const vocab = new Map();
    const scores = [];
    value.model.vocab.forEach((entry, index) => {
    ...
    const findSpecial = (content, fallback) => {
        for (const entry of added) {
            if (isRecord(entry) && entry.content === content && typeof entry.id === "number")
                return entry.id;
        }
...
function encodeWithConfig(config, text) {
...
function unigramSegment(text, vocab, scores, unkId) {
    ...
    const backPiece = new Array(length + 1).fill(undefined);
    bestScore[0] = 0;
