    ...
    return {
        async upsert(entries) {
            return withFileWriteLock(lockPath, async () => {
                ...
                if (manifest.modelId !== model.modelId || manifest.dims !== model.dims)
                    throw new OpenWikiError("INDEX_INCOMPATIBLE", "Vector store manifest does not match the active embedding model.");
                ...
                const segments = [...segmentCounts.entries()].sort(([left], [right]) => left.localeCompare(right)).map(([bucket, count]) => {
                    ...
                    if (contentHash === undefined)
                        throw new OpenWikiError("INVALID_STATE", `Vector segment ${bucket} is missing a content hash.`);
        ...
        async search(vector, limit) {
            if (!Number.isSafeInteger(limit) || limit < 1 || limit > 200)
                throw new OpenWikiError("INVALID_ARGUMENT", "Vector search limit must be between 1 and 200.");
...
function encodeId(id) {
    if (!/^[a-f0-9]{64}$/u.test(id))
        throw new OpenWikiError("INVALID_STATE", "Chunk id must be a 64-character lowercase hex hash.");
...
function decodeBucket(buffer, dims) {
    if (buffer.length < HEADER_BYTES || buffer.toString("ascii", 0, 4) !== SEGMENT_MAGIC)
        throw new OpenWikiError("MODEL_ASSET_CORRUPT", "Vector segment header is invalid.");
    ...
    if (dtype !== 1 || storedDims !== dims)
        throw new OpenWikiError("INDEX_INCOMPATIBLE", "Vector segment dimensions do not match the active embedding model.");
    const vectorsStart = HEADER_BYTES;
    const idsStart = vectorsStart + count * dims;
    if (buffer.length < idsStart + count * ID_BYTES)
        throw new OpenWikiError("MODEL_ASSET_CORRUPT", "Vector segment is truncated.");
    const rows = [];
...
function parseManifestChunk(value) {
    if (!isRecord(value) || !isBucket(value.bucket))
        throw new OpenWikiError("MODEL_ASSET_CORRUPT", "Vector store manifest chunk is invalid.");
