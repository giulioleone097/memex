...
export async function search(request, ports) {
    ...
    if (signals.includes("vector")) {
        if (ports.embedder === undefined || ports.vectorStore === undefined)
            throw new OpenWikiError("MODEL_ASSET_MISSING", "Vector retrieval requires a loaded embedder and vector store.");
        const status = await ports.vectorStore.status();
        if (!status.compatible)
            throw new OpenWikiError("INDEX_INCOMPATIBLE", "Vector store model does not match the loaded embedder.");
    ...
    if (signals.includes("graph")) {
        if (ports.graphIndex === undefined)
            throw new OpenWikiError("NOT_INITIALIZED", "Graph retrieval requires a built graph index.");
...
export function validateSignals(signals) {
    if (signals === undefined)
        return [...ALL_SIGNALS];
    if (signals.length === 0)
        throw new OpenWikiError("INVALID_ARGUMENT", "Signals must include at least one of lexical, vector, graph.");
    const unique = new Set(signals);
    for (const signal of unique)
        if (!ALL_SIGNALS.includes(signal))
            throw new OpenWikiError("INVALID_ARGUMENT", `Unknown retrieval signal: ${signal}.`);
