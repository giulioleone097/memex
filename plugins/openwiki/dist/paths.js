...
export async function resolveWikiLocation(options) {
    if (options.mode === "code") {
        if (!options.root) {
            throw new OpenWikiError("INVALID_ARGUMENT", "Code mode requires a repository root.");
        }
        ...
        return {
            ...
            dataRoot: await resolveProspectivePath(homeDir, path.join(".openwiki", "data", workspaceId)),
        };
    }
    const homeDir = await resolveExistingDirectory(options.homeDir ?? os.homedir());
    ...
    const dataRoot = await resolveProspectivePath(homeDir, path.join(".openwiki", "data", "personal"));
    return {
        mode: "personal",
...
function validatePagePath(page) {
    if (page.length === 0 ||
        ...
        throw new OpenWikiError("PATH_OUTSIDE_ROOT", "Wiki page path must be relative to the wiki root.");
    }
    const segments = page.split("/");
    if (segments.some((segment) => segment.length === 0 || segment === "." || segment === "..")) {
        throw new OpenWikiError("PATH_OUTSIDE_ROOT", "Wiki page path must stay inside the wiki root.");
    }
    if (!page.endsWith(".md")) {
        throw new OpenWikiError("INVALID_ARGUMENT", "Wiki pages must use the .md extension.");
    }
}
...
function assertInside(root, candidate) {
    ...
    throw new OpenWikiError("SYMLINK_ESCAPE", "Resolved path escapes the allowed wiki root.");
}
