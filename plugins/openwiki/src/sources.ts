...
async function assertPersonalWikiUnlocked(location: WikiLocation): Promise<void> {
  ...
  if ((await inspectPath(path.join(location.wikiRoot, ".openwiki.lock"))) !== null) {
    throw new OpenWikiError(
      "LOCKED",
      "OpenWiki is locked; purge cannot infer safe ownership.",
    );
  }
}
...
async function ensureDirectory(directory: string): Promise<void> {
  ...
  if (existing?.isSymbolicLink()) {
    throw new OpenWikiError(
      "SYMLINK_ESCAPE",
      "Private OpenWiki storage must not be a symbolic link.",
    );
  }
...
function mapStorageError(error: unknown, message: string): OpenWikiError {
  if (error instanceof OpenWikiError) {
    return error;
  }
