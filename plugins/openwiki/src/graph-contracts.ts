...
export interface GraphNodeV1 { id: string; kind: GraphNodeKind; path: string; name: string; scope?: string; symbolKind?: string; startLine?: number; endLine?: number; }
...
function parseNode(value: unknown): GraphNodeV1 { const r = object(value, "Graph node must be an object."); assertKeys(r, ["id", "kind", "path", "name", "scope", "symbolKind", "startLine", "endLine"], true); const kind = string(r.kind, "Graph node kind must be a string.") as GraphNodeKind; if (!NODE_KINDS.has(kind)) fail("Graph node kind is unsupported."); const node: GraphNodeV1 = { id: string(r.id, "Graph node id must be a string."), kind, path: relativePath(r.path), name: string(r.name, "Graph node name must be a string.") }; if (r.scope !== undefined) node.scope = string(r.scope, "Graph symbol scope must be a string."); if (r.symbolKind !== undefined) node.symbolKind = string(r.symbolKind, "Graph symbol kind must be a string."); if (r.startLine !== undefined) node.startLine = line(r.startLine); if (r.endLine !== undefined) node.endLine = line(r.endLine); if (node.endLine !== undefined && node.startLine !== undefined && node.endLine < node.startLine) fail("Graph node line range is invalid."); const discriminator = node.startLine === undefined ? undefined : node.scope === undefined ? String(node.startLine) : `${node.scope}\u0000${node.startLine.toString()}`; if (node.id !== createGraphNodeId(node.kind, node.path, node.name, node.symbolKind, discriminator)) fail("Graph node ID does not match its identity fields."); return node; }
