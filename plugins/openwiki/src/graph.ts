...
export async function getGraphStatus(options: GraphOperationBase): Promise<GraphStatusResult> {
  ...
  if (!probed.initialized) return { schemaVersion: 1, action: "status", root: probed.repositoryRoot, available: false, fresh: false, reason: "No recoverable OpenWiki graph manifest exists." };
  let manifest;
  try { manifest = await readManifest(probed.storage); }
  catch { return { schemaVersion: 1, action: "status", root: probed.repositoryRoot, available: false, fresh: false, reason: "No recoverable OpenWiki graph manifest exists." }; }
