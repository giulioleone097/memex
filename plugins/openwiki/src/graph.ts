...
export async function getGraphStatus(options: GraphOperationBase): Promise<GraphStatusResult> {
  const probed = await probeGraphStorage(options.root, options.homeDir); if (!probed.initialized) return { schemaVersion: 1, action: "status", root: probed.repositoryRoot, available: false, fresh: false, reason: "No recoverable OpenWiki graph manifest exists." }; let manifest; try { manifest = await readManifest(probed.storage); } catch { return { schemaVersion: 1, action: "status", root: probed.repositoryRoot, available: false, fresh: false, reason: "No recoverable OpenWiki graph manifest exists." }; } const [evidence, metadata] = await Promise.all([changedRepositoryEvidence(probed.repositoryRoot), enumerateRepositoryMetadata(probed.repositoryRoot, GRAPH_DEFAULTS)]); const indexedPaths = new Set(manifest.shards.map((shard) => shard.path)); const sourceState = await resolveRepositorySourceIds(probed.repositoryRoot, metadata.filter((file) => file.sourceId === undefined || indexedPaths.has(file.path))); const fresh = evidence.head === manifest.source.gitHead && repositoryMetadataFingerprint(sourceState) === manifest.source.dirtyFingerprint; return { schemaVersion: 1, action: "status", root: probed.repositoryRoot, available: true, fresh, ...(fresh ? {} : { reason: evidence.changeState === "working-tree" ? "Repository content changed since the last graph build." : "Repository HEAD changed since the last graph build." }), ...(manifest.source.gitHead === undefined ? {} : { indexedHead: manifest.source.gitHead }), ...(evidence.head === undefined ? {} : { currentHead: evidence.head }), counts: manifest.counts, generatedAt: manifest.generatedAt };
}
