...
export function queryGraph(graph: CodeGraphV1, options: QueryGraphOptions): GraphResult {
  const query = options.query.trim(); if (query.length === 0) throw new OpenWikiError("INVALID_ARGUMENT", "Graph query must not be empty.");
...
export function entityLimit(value: number | undefined): number { const result = value ?? GRAPH_DEFAULTS.defaultEntityLimit; if (!Number.isSafeInteger(result) || result < 1 || result > GRAPH_DEFAULTS.maxEntityLimit) throw new OpenWikiError("INVALID_ARGUMENT", "Graph entity limit must be between 1 and 100."); return result; }
export function responseLimit(value: number | undefined): number { const result = value ?? GRAPH_DEFAULTS.defaultResponseBytes; if (!Number.isSafeInteger(result) || result < 256 || result > GRAPH_DEFAULTS.maxResponseBytes) throw new OpenWikiError("INVALID_ARGUMENT", "Graph response limit must be between 256 and 65536."); return result; }
...
function traversalDepth(value: number | undefined): number { const result = value ?? GRAPH_DEFAULTS.maxTraversalDepth; if (!Number.isSafeInteger(result) || result < 1 || result > GRAPH_DEFAULTS.maxTraversalDepth) throw new OpenWikiError("INVALID_ARGUMENT", "Graph traversal depth must be between 1 and 5."); return result; }
