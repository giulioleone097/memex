...
function parseManifestEntry(value: unknown): VendorManifestEntry {
  ...
  parts.forEach((part, index) => {
    if (!part.path.endsWith(`.part${String(index)}`)) throw new OpenWikiError("MODEL_ASSET_CORRUPT", `Vendor asset manifest parts for ${entryPath} must be declared in order part0, part1, ....`);
  });
