...
export async function openGraphIndexGeneration(
  ...
  return {
    ...
    async rankedCandidates(query, limit) {
      ...
      if (normalized.length === 0 || queryTokens.length === 0) throw new OpenWikiError("INVALID_ARGUMENT", "Graph query must not be empty.");
...
async function writeBuckets<T extends NodeBucket | EdgeRecordBucket | EdgeBucket | CandidateBucket | PathBucket>(
  ...
  for (const name of names) {
    ...
    if (values === undefined) throw new OpenWikiError("INVALID_STATE", "Graph bucket disappeared during serialization.");
  ...
  return {
    ...
    async write(relative, content) {
      ...
      try {
        ...
        if (error instanceof Error && "code" in error && error.code === "EEXIST") throw new OpenWikiError("INVALID_STATE", "Immutable graph generation already contains this entry.");
        throw error;
      } finally {
...
function parseEdgeListBucket(value: unknown): Map<string, GraphEdgeV1[]> {
  return parseRecord(value, (entry) => {
    if (!Array.isArray(entry)) throw new OpenWikiError("INVALID_STATE", "Graph edge bucket is invalid.");
...
function parseStringListBucket(value: unknown): Map<string, string[]> {
  return parseRecord(value, (entry) => {
    if (!Array.isArray(entry) || !entry.every((item) => typeof item === "string")) throw new OpenWikiError("INVALID_STATE", "Graph index bucket is invalid.");
    return entry;
  });
}

function parseRecord<T>(value: unknown, parse: (entry: unknown) => T): Map<string, T> {
  if (!isRecord(value)) throw new OpenWikiError("INVALID_STATE", "Graph index bucket is invalid.");
...
function parseDiagnostic(value: unknown): GraphDiagnosticV1 {
  if (!isRecord(value) || typeof value.path !== "string" || typeof value.code !== "string" || typeof value.message !== "string") throw new OpenWikiError("INVALID_STATE", "Graph architecture index is invalid.");
  return { path: value.path, code: value.code, message: value.message };
}

function parseHub(value: unknown): { id: string; degree: number } {
  if (!isRecord(value) || typeof value.id !== "string" || !nonNegativeInteger(value.degree)) throw new OpenWikiError("INVALID_STATE", "Graph architecture index is invalid.");
...
function parseBuckets(value: unknown): string[] {
  ...
  const names: string[] = [];
  for (const entry of value) { if (typeof entry !== "string" || !/^[a-f0-9]$/u.test(entry)) throw new OpenWikiError("INVALID_STATE", "Graph index bucket manifest is invalid."); names.push(entry); }
