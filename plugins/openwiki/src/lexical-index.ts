...
function parsePostings(value: Record<string, unknown>): Record<string, Posting[]> {
  ...
  for (const [term, entry] of Object.entries(value)) {
    ...
    result[term] = entry.map((posting) => {
      if (!isRecord(posting) || typeof posting.id !== "string" || !isNonNegativeInteger(posting.frequency)) throw new OpenWikiError("INVALID_STATE", "Lexical index manifest is invalid.");
