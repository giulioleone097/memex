...
OpenWiki never stores provider tokens. Gmail, Notion, Slack, web-search, and X authentication remains in tools already authorized by Codex or Claude Code. The plugin stores only a connector kind and optional non-secret account hint or query provenance after validation and redaction.
...
Use `openwiki-ops` to inspect exact purge scope before deletion. Supported scopes remove private raw data, schedules, or the personal wiki without following symlinks. Uninstalling the plugin does not delete user wiki or private data. Partial deletion must be reported path by path.
