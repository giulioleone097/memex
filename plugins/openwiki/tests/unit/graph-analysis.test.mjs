...
async function temporaryRoot(label) {
  const root = await mkdtemp(path.join(os.tmpdir(), `openwiki-graph-analysis-${label}-`));
  roots.push(root);
  return root;
}
...
describe("graph semantic analysis", () => {
  ...
  test("graph: maps dependency cycles and computes transitive inbound dependents deterministically", async () => {
    ...
    await assert.rejects(getArchitectureMap({ root, homeDir, limit: 100, responseByteLimit: 256 }), (error) => error instanceof OpenWikiError && error.code === "SOURCE_TOO_LARGE");
  });
