...
describe("contracts", () => {
  ...
  test("contracts: allows arbitrary metadata keys and rejects unknown structural keys", () => {
    ...
    for (const invalid of [
      ...
      assert.equal(captureOpenWikiError(() => parseWikiState(invalid)).code, "INVALID_STATE");
    }
    ...
    for (const invalid of [
      ...
      assert.equal(
        captureOpenWikiError(() => parseSourceEnvelope(invalid)).code,
        "INVALID_ARGUMENT",
      );
    }
  });
  ...
  test("contracts: rejects unknown schema versions", () => {
    const stateError = captureOpenWikiError(() =>
    ...
    const envelopeError = captureOpenWikiError(() =>
  ...
  test("contracts: rejects invalid timestamps at every timestamp boundary", () => {
    for (const state of [
      ...
      assert.equal(captureOpenWikiError(() => parseWikiState(state)).code, "INVALID_STATE");
    }
    ...
    for (const envelope of [
      ...
      assert.equal(
        captureOpenWikiError(() => parseSourceEnvelope(envelope)).code,
        "INVALID_ARGUMENT",
      );
    }
  });
  ...
  test("contracts: rejects duplicate external IDs", () => {
    ...
    const error = captureOpenWikiError(() => parseSourceEnvelope(envelope));
  ...
  test("contracts: enforces item, text, metadata, and total envelope limits", () => {
    ...
    assert.equal(
      captureOpenWikiError(() => parseSourceEnvelope(oversizedEnvelope)).code,
      "SOURCE_TOO_LARGE",
    );
  });

  test("contracts: serializes typed errors to stable safe JSON", () => {
    const error = new OpenWikiError("INVALID_STATE", "Wiki state is invalid.");

    assert.equal(error.name, "OpenWikiError");
    assert.deepEqual(error.toJSON(), {
      code: "INVALID_STATE",
