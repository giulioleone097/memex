...
export function makeTemporaryRoot(t, label) {
  const root = mkdtempSync(join(tmpdir(), `openwiki adapter ${label} `));
...
export function initializeGitRepository(root, files = {}) {
  ...
  runGit(root, ["config", "user.email", "openwiki@example.test"]);
  runGit(root, ["config", "user.name", "OpenWiki Adapter Test"]);
