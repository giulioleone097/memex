...
const REQUIRED_PLUGIN_FILES = [
  ...
  "skills/openwiki-init/SKILL.md",
  "skills/openwiki-update/SKILL.md",
  "skills/openwiki-query/SKILL.md",
  "skills/openwiki-ingest/SKILL.md",
  "skills/openwiki-ops/SKILL.md",
  "src/cli.ts",
  "src/mcp.ts",
  "src/hook.ts",
  "package.json",
...
function validateMarketplaceDocuments(repositoryRoot, pluginRoot, documents, findings) {
  ...
  if (codex) {
    ...
    if (entry?.source?.source !== "local" || sourcePath !== "./plugins/openwiki") {
      findings.push(
        createFinding(
          "SOURCE_PATH_MISMATCH",
          ".agents/plugins/marketplace.json",
          "Codex OpenWiki source must be local ./plugins/openwiki.",
        ),
      );
    }
      ...
      if (!isWithin(repositoryRoot, resolved)) {
        ...
        findings.push(
          createFinding(
            "SOURCE_PATH_MISMATCH",
            ".agents/plugins/marketplace.json",
            "Codex marketplace source does not resolve to plugins/openwiki.",
          ),
        );
      }
    }
  ...
  if (claude) {
    ...
    if (sourcePath !== "./plugins/openwiki") {
      findings.push(
        createFinding(
          "SOURCE_PATH_MISMATCH",
          ".claude-plugin/marketplace.json",
          "Claude OpenWiki source must be ./plugins/openwiki.",
        ),
      );
    }
    if (typeof sourcePath === "string") {
      ...
      if (!isWithin(repositoryRoot, resolved)) {
        ...
        findings.push(
          createFinding(
            "SOURCE_PATH_MISMATCH",
            ".claude-plugin/marketplace.json",
            "Claude marketplace source does not resolve to plugins/openwiki.",
          ),
        );
      }
    }
...
function validateAttribution(repositoryRoot, pluginRoot, findings) {
  ...
  if (exists(noticesPath)) {
    ...
    if (!/OpenWiki/iu.test(content) || !/MIT/iu.test(content)) {
      findings.push(
        createFinding(
          "INVALID_ATTRIBUTION",
          relative(repositoryRoot, noticesPath),
          "Third-party notices must identify OpenWiki and its MIT license.",
        ),
      );
    }
  }
...
function validateRepository(repositoryRoot) {
  const pluginRoot = pluginPath(repositoryRoot, "plugins/openwiki");
  const findings = [];
